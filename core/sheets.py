...
# корня высыпало бы все названия поверх собственных текстов владельца. В коде
# серии было написано, что папка «одна на всё, её нельзя удалить», — и это
...
# от «/», обзор читал только верхний уровень. Владелец просил дерево ещё в
# брифе редактора («папки под папки, как в заметках»), а серия без него
...
def folder_delete(fid) -> dict:
    ...
    то же. С деревом — уже нет: удалив «Серии/Пепел», владелец высыпал бы весь
    ...
    # тот же день). Сначала я запретил его совсем — и владелец справедливо
    # сказал «не могу папки удалять»: у него это была единственная папка, к
