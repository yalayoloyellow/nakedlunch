...
#
# Требование (2026-08-03): каждому звену пайплайна выбирается готовая строфа и готовый
# профиль настроек — ничего не настраивается заново..
#
...
def resolve_chain(chain: list[dict], base_knobs: dict) -> list[dict]:
    ...
    for item in chain:
        ...
        # самая строфа, стоящая в песне второй раз. Требование: механизм обязан уметь хуки, хуковые припевы и подобные элементы..
        #
...
def _combo_score(q_sum: float, j_sum: float, rep_sum: float, n: int,
    ...
    другим: требование (2026-08-03): отдельной оценки качества не нужно — выдача и так должна быть
    максимально качественной; «если я так стыки размечаю, зачем мне
...
def assemble(links: list[dict], pools: dict, junctions: list[str], runs: int,
    ...
    общие леммы между строфами. Требование: повтор запрещён только при соответствующей настройке; если повтор нужен — он
    работает.."""
    n_links = len(links)
