...
def summary() -> dict:
    ...
    return {
        ...
        "живое": {
            ...
            # ХИТ-РЕЙТ ПО ЖИВЫМ ПОКАЗАМ — единственное число, по которому видно,
            # стал ли отбор лучше. Соседнее `favorites.rate_pct` делит на ВСЕ
            # показы, включая машинные, и потому занижает: 0.81% против 0.87%
            # на замере 2026-08-20. Разница невелика сейчас, но знаменатель там
            # растёт от любого прогона нагрузки, а этот — только от работы.
            "показано_строк": строк_живьём,
            "хит_рейт_pct": (round(100 * total_favorited / строк_живьём, 2)
                             if строк_живьём else None),
            "одна_строка_из": (round(строк_живьём / total_favorited)
                               if total_favorited else None),
        },
