...
def summary() -> dict:
    ...
    живые_показы, _ = разделить_на_живое_и_машину(shown)
    строк_живьём = sum(e.get("count", 0) for e in живые_показы)
    ж_тема = ж_наборы = 0
    ...
    return {
        ...
        "живое": {
            ...
            # НАДГРОБИЕ: `показано_строк` / `хит_рейт_pct` / `одна_строка_из`
            # УДАЛЕНЫ 2026-08-21 по решению владельца: «похуй на хитрейт, я уже
            # знаю что работает а что нет, не надо мерить сколько я сохранил».
            #
            # Это была ДОЛЯ СОХРАНЁННЫХ строк от показанных живьём (0.87% на
            # 2026-08-20 — одна строка из 115), и я держала её главным числом
            # проекта. Владелец отменил саму постановку: сохранение — его
            # частный жест, а не оценка отбора, и превращать его в счёт значит
            # мерить не программу, а человека.
            #
            # ОСТАЛЬНОЙ БЛОК `живое` ОСТАЁТСЯ, и это не полумера: он отделяет
            # человеческие прогоны от машинных всплесков, без чего ЛЮБОЕ число
            # журнала врёт (на этом обожглись трижды — см. шапку файла). Он
            # считает, ЧТО делал человек, а не насколько ему понравилось.
        },
        ...
        "favorites": {
            ...
            # `rate_pct` (доля сохранённых от всех показов) снята там же и по
            # той же причине — см. надгробие в блоке «живое» выше.
...
# `banality` (2026-08-20), `melody` и `flow` (2026-08-21) ОСТАЮТСЯ В ВЫГРУЗКЕ,
# ХОТЯ РУЧЕК БОЛЬШЕ НЕТ. Журнал —
