...
def _записать_целиком(путь: Path, текст: str) -> None:
    ...
    поверх себя: `state_path.write_text(json.dumps(...))`. На рабочем корпусе
...
class NakedLunchStore:
    ...
    def _load(self) -> None:
        ...
        if self.state_path.exists():
            ...
            try:
                ...
                loaded_full = True
            # ДВЕ ПРИЧИНЫ — ДВА РАЗНЫХ ОТВЕТА (2026-09-02). Здесь `OSError`
            # стоял в одном ряду с ошибками разбора, и файл, к которому просто
            # не дали доступ, объявлялся ПОВРЕЖДЁННЫМ — с советом «почини или
            # удали вручную». Совет указывал на state.json в сотни мегабайт со
            # всеми книгами; послушайся он совета — потерял бы всё,
            # притом что с файлом ничего не случилось. Отказ, который врёт о
            # причине, хуже отказа: он ещё и подсказывает неверное действие.
                ...
                raise RuntimeError(
                    f"нет доступа к складу: {self.state_path}\n"
                    f"Сам файл цел — программе не дали его прочитать. Выдай ей "
                    f"доступ к папке «Документы» в Системных настройках → "
                    f"Конфиденциальность и безопасность → Доступ к файлам и "
                    f"папкам, и запусти заново. УДАЛЯТЬ ЗДЕСЬ НЕЧЕГО.") from e
            except OSError as e:
                raise RuntimeError(
                    f"склад не читается: {self.state_path}\n"
                    f"Файл на месте, но система отдала ошибку «{e.strerror or e}». "
                    f"Так бывает, когда диск отключён или занят другим процессом. "
                    f"Сам файл при этом мог остаться целым — прежде чем что-то "
                    f"удалять, проверь, что он открывается.") from e
        if not loaded_full:
            self.state = State()
    ...
    def add_corpus(
        ...
        # БЛИЗНЕЦЫ СВЕРЯЮТСЯ СО ВСЕМ СКЛАДОМ, А НЕ ТОЛЬКО ВНУТРИ КНИГИ
        # (2026-08-21). Резак видит только свою книгу и дедупит внутри неё; одна
        # и та же фраза из двух разных книг проходила насквозь. Замер на девяти
        # живых книгах в одном складе: 1 696 таких пар на 571 496 фрагментов
        # (0.30%) — и кнопка «почистить» после заливки снова находила работу,
        # то есть заливка переставала быть самодостаточной.
        #
        # Ключ ТОТ ЖЕ, что у резака и у чистки склада (`СЛОВО` по нижнему
        # регистру): три разных ключа в трёх местах разошлись бы в первую же
        # волну правок — на этом тут уже обжигались.
        # ХЭШ, А НЕ САМ КЛЮЧ — ЗАМЕРЕНО НА БОЕВОМ СКЛАДЕ (2026-08-21).
        # Множество кортежей слов по 2.4 млн фрагментов весит 1 213 МБ, строк —
        # 370, хэшей — 145. Время у всех троих одинаковое (~16 с, его съедает
        # сам разбор на слова). Приложение уже держит navec на 1 479 МБ, и
        # сборщик рифм на 16 ГБ периодически убивает система — лишний гигабайт
        # тут покупал бы ноль.
        #
        # Цена — теоретическая коллизия: при 2.4 млн ключей и 64-битном хэше
        # вероятность хоть одной ≈ 1.6e-7, а последствие — один выброшенный
        ...
        if not frags:
            ...
            # Под общим кодом человек читал бы «нарезать нечего, слишком
    ...
    # фрагментов (2026-07-19, просьба): исходный текст книги после
    ...
    #
    # ЗАЧЕМ. Требование: «и обогащение и очистка
    ...
    def почистить(self, применить: bool = True, прогресс=None) -> Dict[str, int]:
        ...
        НЕ МОЖЕТ. Проход идёт две минуты, и правило (2026-08-21):
        ...
        итог = {"было": len(self.state.fragments), "указатель": 0,
        ...
        for _н, f in enumerate(self.state.fragments):
            ...
            #
            # Разбор fb2 снимает их с 2026-08-21, но девять рабочих книг уже
            ...
            if м != т:
                ...
                итог["маркер_снят"] += 1
                т = м
            # НАДГРОБИЕ: ПОФРАГМЕНТНОЕ СНЯТИЕ ИМЁН УБРАНО 2026-08-27.
            #
            # Здесь стояло `б = strip_full_names(т)` со счётчиком
            # «имя_вырезано». Шаг был нужен корпусам, нарезанным ДО того, как
            # снятие имён встало в заливку (2026-08-21, `clean_text` →
            # `strip_full_names` по целому тексту): их фрагменты имён не
            # проходили никогда, и кнопка доводила старое до нового вида.
            #
            # После пересборки рабочего корпуса из исходников (2026-08-26)
            # таких корпусов не осталось, и шаг НЕ ЧИСТИЛ, А ПОРТИЛ. Замер на
            # свежезалитом складе всех 41 книги: 26 срабатываний, и все —
            # порча. Резак срезает открывающую кавычку с края фрагмента,
            # название («Евгений Онегин», «Сергею Есенину», «Андрей Рублев»)
            # теряет примету названия, и пофрагментный проход вырезал его как
            # «имя». По целому тексту эти места видны кавычкой и НЕ снимаются —
            # правее из двух гранулярностей текстовая, а она уже отработала на
            # заливке.
            #
            # Вернуть шаг — значит снова резать названия. Если однажды сюда
            # приедет склад, нарезанный до 2026-08-21, чинить его надо
            # ПЕРЕЗАЛИВКОЙ из исходника, а не пофрагментным ножом.
            х = подрезать(т)
            ...
            if т is not f.text and is_index_junk(т):
                итог["указатель"] += 1
                continue
            # ЦИФРА В СТРОКЕ — СТРОКА НЕ НУЖНА (2026-08-21). То же правило, что
            # у резака (`cutter.есть_цифра`), а не своя копия: старые книги
            # нарезаны до него, и кнопка обязана довести их до того же вида.
            # Судья — избранное: из 62 сохранённых им строк цифра
            # есть в НУЛЕ.
            #
            # ПОСЛЕ ВСЕХ ПРАВОК, А НЕ ДО НИХ. Поставленная в начало, проверка
            # выбрасывала целиком строку с обрубленным маркером («…Едином»[709»)
            # вместо того, чтобы дать её ПОЧИНИТЬ до «…Едином»». Поймано тремя
    ...
    def _снять_близнецов(self, фрагменты: List[Fragment], итог: Dict[str, int],
        ...
        значит хозяев первой достаточно, чтобы найти всех контейнеров.
        ...
        for i, w in enumerate(слова):
            ...
            for j in первая.get(w[:5], ()):
                if j == i or двойник[j] or обломок[j]:
                    continue
                # ТОЛЬКО ВНУТРИ ОДНОЙ КНИГИ (2026-08-21). Вложенность значит
                # разное в разных местах, и это не придирка, а замер.
                #
                # ВНУТРИ книги короткая строка — обрубок длинной, рождённый
                # скользящим окном: «за каждой обиды нельзя отрекаться от
                # друга» это обрезанное «Из-за каждой обиды…». Мусор.
                #
                # МЕЖДУ книгами то же правило ловит ОБЩИЕ ОБОРОТЫ ЯЗЫКА. На
                # девяти живых книгах оно нашло ровно 23 таких пары, и вот они
                # целиком: «во что бы то ни стало», «ни с того ни с сего»,
                # «если уж на то пошло», «что я имею в виду», «в то же время»…
                # Ни одного обрубка — только идиомы, которые два автора
                # употребили независимо. Резать их незачем: это язык, а не
                # след разреза. Резак (`cutter._снять_обломки`) работал внутри
                # книги с самого начала; кнопка расходилась с ним.
                if фрагменты[j].corpus_id != фрагменты[i].corpus_id:
                    continue
                b = слова[j]
                if len(b) <= len(w):
                    continue
                ...
                #
                # Сквозной прогон по избранному показал, что правило
