...
# ---------------------------------------------------------------------------
# СЧЁТ СОДЕРЖАТЕЛЬНЫХ СЛОВ — СВОЙ СПИСОК ЧАСТЕЙ РЕЧИ (2026-08-18)
#
# ЗАЧЕМ ОТДЕЛЬНЫЙ СПИСОК, А НЕ РАСШИРЕНИЕ `_CONTENT_POS`. У того списка ДВЕ
# работы, и они требуют разного. Как множество ЛЕММ он кормит тему, разнообразие
# соседей, тавтологию и эхо в избранном — там наречия и сравнительные степени
# лишние: «очень», «дальше», «уже» совпадали бы у всего подряд и размывали бы и
# тему, и близость строк. А как СЧЁТЧИК содержательности он отвечает на другой
# вопрос — «есть ли в строке хоть что-то, кроме служебных слов и имён», — и там
# наречие полноценное слово.
#
# ЦЕНА МОЛЧАНИЯ ЗАМЕРЕНА. Порогом «не меньше двух» сейчас режется 8.62% годных
# строк, и 39% из них — не мусор: они не проходят только потому, что их слова
# не той части речи. На корпусе это ~79 900 строк. Вот что выбрасывается:
#
#     «Дальше — хуже» · «Нет у меня денег» · «ты очень ошибаешься»
#     «как нельзя лучше. Так» · «Надо же, как я ошибался»
#
# То есть короткие разговорные строки — ровно те, ради которых пишут песни, —
# летели в корзину наравне с архивным шифром «РГАЛИ. Ф. 155. Оп».
#
# COMP это сравнительная степень («дальше», «хуже», «больше»), PRED —
# предикатив («надо», «жаль», «пора»). Обе части речи знаменательные, обе
# держат на себе целую короткую строку.
_СЧЁТНЫЕ_POS = _CONTENT_POS + ("ADVB", "COMP", "NUMR", "PRED")


@lru_cache(maxsize=200_000)
def _счётная_лемма(tok: str) -> str | None:
    """Как `_lemma_of`, но по расширенному списку. Отдельный кэш нарочно: общий
    отдавал бы разные ответы на один токен в зависимости от того, кто спросил
    последним, — то есть молча зависел бы от порядка вызовов."""
    top = _morph().parse(tok)[0]
    if top.tag.POS in _СЧЁТНЫЕ_POS:
        return top.normal_form
    for p in _morph().parse(tok):
        if p.tag.POS in _СЧЁТНЫЕ_POS and p.score >= _HOMOGRAPH_FLOOR:
            return p.normal_form
    return None


def содержательных(text: str) -> int:
    """Сколько в строке слов, которые что-то значат: не служебные и не имена.

    Разбор токенов ДОСЛОВНО тот же, что у `lemmatize_pairs` — иначе два счёта
    одного и того же разошлись бы на пунктуации, а на ней в этом проекте уже
    теряли слова целиком (см. `_STRIP_CHARS`).
    """
    n = 0
    for tok in text.replace("—", " ").replace("–", " ").split():
        сырое = tok.strip(_STRIP_CHARS)
        if not сырое:
            continue
        if _счётная_лемма(сырое.lower()) and not имя_ли(сырое):
            n += 1
    return n
...
class Corpus:
    ...
    def mark_shown(self, items, theme: str = "", семя=None) -> None:
        ...
        for it in items:
            ...
            # `в_корпусе` — ТЕКСТ, КОТОРЫМ ЗАПИСЬ СВЕРЯЕТСЯ С КОРПУСОМ.
            # Обычно он же и показан. Но с 2026-09-01 отбор умеет подрезать
            # длинную строку под слоговую вилку: человек видит обрезок, а в
            # корпусе лежит полная строка. Прятать надо ПОЛНУЮ — иначе она
            # останется непоказанной и выпадет снова, уже целиком.
            запись = {"text": text, "template": it.get("template", ""),
                      "tags": tags, "shown_at": now, "restored_at": None,
                      "seed": семя}
            исходный = it.get("_исходный")
            if исходный and исходный != text:
                запись["в_корпусе"] = исходный
            # НОМЕР СТРОКИ В ИНДЕКСЕ (2026-09-03). История прячет показанное по
            # ТЕКСТУ, и чтобы вычесть её из выдачи, тексты переводились в номера
            # словарём на 2.3 млн ключей — 5.4 секунды на каждый запуск, и это
            # была вся цена первой генерации. Номер известен движку даром, он
            # приходит в строке полем `_ном`; записанный здесь, он снимает
            # словарь совсем.
            #
            # ВЕРЕН ОН ТОЛЬКО ДЛЯ СВОЕГО ИНДЕКСА, поэтому и хранится вместе со
            # штампом состава (`index_stamp` рядом, один на весь файл):
            # перепечка меняет номера, и `перепривязать` их пересчитывает. Без
            # этой привязки номер стал бы указателем в никуда — худшим видом
            # ускорения, тихо прячущим не ту строку.
            ном = it.get("_ном")
            if isinstance(ном, int) and ном >= 0:
                запись["ном"] = ном
            self.history.append(запись)
            have.add(text)
    ...
    def скрытые_номера(self, штамп: str, кроме_семени=None) -> tuple[set, set]:
        """(номера строк, тексты без номера) — для быстрой маски истории.

        ЗАЧЕМ. Прятать показанное можно по тексту или по номеру строки. По
        тексту — значит перевести его в номер, а перевод стоил словаря на весь
        корпус: 5.4 секунды при каждом запуске, и это была вся цена первой
        генерации после того, как маску пула перевели на колонку `src`.
        Номер, записанный при показе, переводить не надо вовсе.

        СОСТАВ СКРЫТОГО РЕШАЕТ `hidden_set`, А НЕ ЭТОТ МЕТОД. Он один знает про
        срок хранения, восстановленные записи и избранное, и заводить второе
        такое место значило бы развести две правды о том, что видно. Здесь
        только перевод: у каждой уже отобранной строки спрашивается её номер.

        ШТАМП СВЕРЯЕТСЯ ЗДЕСЬ. Номер верен только для того состава строк, из
        которого взят; разошёлся штамп — номерам грош цена, и честнее отдать
        всё текстами, чем спрятать не ту строку. Тот же случай, что с
        `в_корпусе`: указатель, переживший своё основание, опаснее
        отсутствующего.

        Второй возврат — ТЕКСТЫ тех, у кого номера нет: записей, снятых до
        2026-09-03. Их зовущий переведёт словарём, как раньше; после первой
        перепривязки хвост пустеет навсегда.
        """
        скрыто = self.hidden_set()
        if кроме_семени is not None:
            скрыто = скрыто - self.тексты_прогона(кроме_семени)
        if not штамп or штамп != self.index_stamp:
            return set(), скрыто
        по_ключу = {}
        for з in self.history:
            по_ключу[self._ключ(з)] = з.get("ном")
        for з in self.accepted:
            по_ключу.setdefault(self._ключ(з), з.get("ном"))
        номера, тексты = set(), set()
        for к in скрыто:
            н = по_ключу.get(к)
            if isinstance(н, int):
                # −1 — «проверено, в индексе такой строки нет». Она не может
                # попасть в выдачу по той же причине, по которой не нашлась, и
                # места в маске ей не нужно вовсе. Без этой пометки сироты
                # держали бы словарь живым вечно: их номер не появится никогда,
                # и каждый запуск заново платил бы 5.4 с за перевод того, чего
                # нет. Сирот у живой истории 2 981 из 5 355 — больше половины.
                if н >= 0:
                    номера.add(н)
                continue
            тексты.add(к)
        return номера, тексты
    ...
    def перепривязать(self, штамп: str, есть, подрезать, номер=None) -> dict:
        ...
        if not штамп or штамп == "без индекса" or штамп == self.index_stamp:
            return итог
        итог["нужна"] = True
        # НОМЕРА СТРОК ПЕРЕСЧИТЫВАЮТСЯ ЗДЕСЬ ЖЕ (2026-09-03).
        #
        # Запись хранит номер строки в индексе (`ном`), и по нему история
        # вычитается из выдачи без словаря на 2.3 млн ключей. Номер верен
        # только для СВОЕГО состава строк — а перепривязка случается ровно
        # тогда, когда состав сменился. Значит здесь номера обязаны либо
        # обновиться, либо исчезнуть: указатель, переживший своё основание,
        # спрячет не ту строку, и это хуже, чем не спрятать ничего.
        #
        # `номер(текст) -> int | None` приходит снаружи по той же причине, что
        # и `есть`: корпус не знает про индекс и не должен.
        for имя, записи in (("история", self.history), ("избранное", self.accepted)):
            for з in записи:
                ...
                if короче and короче != текст and есть(короче):
                    ...
                    итог["сироты"] += 1
                    з["ном"] = -1           # проверено: в индексе такой строки нет
        self.index_stamp = штамп
        return итог

    def проставить_номера(self, номер) -> int:
        """Дописать номер строки записям, у которых его нет. Сколько дописано.

        РАЗОВЫЙ ПЕРЕВОД, А НЕ ПЕРЕПРИВЯЗКА. Перепривязка идёт при СМЕНЕ состава
        строк и переписывает всё; этот проход нужен один раз в жизни — записям,
        снятым до 2026-09-03, когда номер ещё не хранился. Состав при этом тот
        же, поэтому и трогать больше нечего: тексты на месте, не хватает только
        указателя.

        Пока такие записи есть, история переводится в номера словарём на 2.3 млн
        ключей — 5.4 секунды при каждом запуске. После прохода словарь не нужен
        никогда: у всех записей есть номер, а у сирот его и не должно быть.
        """
        сделано = 0
        for записи in (self.history, self.accepted):
            for з in записи:
                if isinstance(з.get("ном"), int):
                    continue
                н = номер(self._ключ(з))
                if isinstance(н, int) and н >= 0:
                    з["ном"] = н
                    сделано += 1
                else:
                    # СИРОТА — И ЭТО ЗАПИСЫВАЕТСЯ. Строки в индексе нет: она
                    # либо из прежней нарезки, либо из удалённой книги. Пометка
                    # −1 говорит «проверено», и следующий запуск не станет
                    # строить словарь ради того, чего нет.
                    з["ном"] = -1
        return сделано

    @staticmethod
    def _пометить_номер(запись: dict, номер) -> None:
        """Проставить (или снять) номер строки после перепривязки.

        Снять — обязательная половина: старый номер указывал в прежний состав
        строк, и оставить его значило бы прятать чужую строку молча."""
        if номер is None:
            запись.pop("ном", None)
            return
        н = номер(Corpus._ключ(запись))
        запись["ном"] = н if isinstance(н, int) and н >= 0 else -1
