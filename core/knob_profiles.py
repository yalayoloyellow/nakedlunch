...
#
# Владелец 2026-08-03: «ты имеешь сохранение строф, профили настроек и
# цепочек… можно делать строфы, и можно сделать профиль настроек, то есть как
# расположены крутилки. И то, и то можно ставить отдельно».
#
...
# (файл, который владелец может открыть руками, не заслуживает доверия ни в
...
# Стартовые профили. Не «вкусовые пресеты» (их владелец соберёт сам под свой
...
#
# «Обычный» — ИЗМЕРЕННЫЕ положения владельца из data/settings.json (nl_params
...
def custom() -> list[dict]:
    """Свои профили владельца, или [] если файла нет и он нечитаем. Никогда не
...
def by_name(name: str) -> dict | None:
    ...
    это сознательный оверрайд владельца (то же правило, что у форм строф в
