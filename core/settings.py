...
# НАДГРОБИЕ 2026-08-18: `nl_chain` УБРАН ИЗ БЕЛОГО СПИСКА.
#
# Это была ЖИВАЯ цепочка меню «Пайплайн» — «где закрыл, там открыл». Пайплайн
# вырезан целиком: 29 прогонов цепи против 587 одиночных строф за 10 живых
# дней (1 к 20), медиана 24.2 с, девяностый процентиль 98.7 с. Серия ушла тем
# же решением от 2026-08-18, но по ней числа нет — она в журнал
# событий не писалась вовсе.
#
# Ключ `nl_chain_profiles` (полка цепочек) убран ещё Раундом 50, а сама полка
...
# причина, по которой выше ушёл `nl_chain`. Решением 2026-08-28.
#
...
def _nl_params(raw) -> dict:
    ...
    mode = raw.get("mode")
    # ПОЛОСЫ РЕДКОСТИ ПРОХОДЯТ ЗДЕСЬ ЖЕ (2026-08-26), И ЭТО НЕ МЕЛОЧЬ.
    #
    # Канон крутилок числовой — (низ, верх, дефолт), то есть один отрезок.
    # Полосы это несколько несмежных отрезков шкалы, числом их не выразить, и
    # `clean.knob_params` выбросил бы их молча.
    #
    # Ровно та ловушка, о которой предупреждает `write` в этом же файле:
    # «фильтра два, и второй молча съедал ключ, пропущенный в первом».
    # Роут /api/settings полосы уже сохранял — а здесь они терялись, и на
    # живой проверке это выглядело так: полосу набрал, окно перезапустил,
    # полосы нет.
    полосы = raw.get("полосы") if isinstance(raw.get("полосы"), dict) else {}
    # ДОЛИ ПРОХОДЯТ ЗДЕСЬ ЖЕ (2026-08-29), И ЭТО НЕ МЕЛОЧЬ — это ровно та
    # ловушка, о которой предупреждает `write` в этом файле: «фильтра два, и
    # второй молча съедал ключ, пропущенный в первом». Полосы через это уже
    # проходили: набрал полосу, перезапустил окно — её нет.
    ...
    return {"mode": mode if mode in clean.KNOB_MODES else clean.MODE_ALGO,
            ...
            # ОСИ БЕРУТСЯ ИЗ КАНОНА (2026-08-30). Здесь стоял свой список
            # осей — третий по счёту, — и он-то и съел полосу «плотность
            # звука»: роут её сохранял, а этот фильтр молча выбрасывал.
            # Список теперь один, `clean.ОСИ_ПОЛОС`/`ОСИ_ДОЛЕЙ`.
            "полосы": clean.полосы_из(полосы),
            "доли": clean.доли_из(доли_)}
