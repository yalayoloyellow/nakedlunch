...
from clean import SHORTLIST_MAX
...
# взрывов»). Собираются ОТДЕЛЬНЫЕ находки, а соседей выбрасывает.
...
# референсного текста, 0.35), а ПОЛОСОЙ-ФИЛЬТРОМ с равновероятным жребием
...
# Обязательное слово вырезано вместе с темой по требованию («вырезать всё
...
def _classic_pool(knobs, corpus, nl_fragments, *, hidden, no_mat, only_mat, clausula, cap,
    ...
    if _idx is not None:
        ...
        # требование — «классика вообще все настройки отключает и просто
...
_ПАЧКА_НА_СТРОКУ = 8
_ПАЧКА_МИН = 32
_КОРОТКАЯ_ВЫДАЧА_МАКС = SHORTLIST_MAX


def _размер_полных_строф(запрошено: int, размер_строфы: int) -> tuple[int, int]:
    """Вернуть (строк к сборке, строф к сборке) без неполного хвоста.

    `shortlist` — общая ручка и приходит в строках, а прямая тяга умеет
    отдавать только целые строфы. Округляем вверх, чтобы маленький запрос не
    превратился в ноль; если округление упирается в общий предел — берём
    максимально возможное число полных строф.
    """
    размер_строфы = max(1, int(размер_строфы))
    запрошено = max(1, int(запрошено))
    строф = max(1, (запрошено + размер_строфы - 1) // размер_строфы)
    # Форма сама может быть длиннее общего буфера. В этом случае одна полная
    # строфа всё равно полезнее нулевого запроса: лимит относится к числу
    # строк выдачи, а не должен делать существующую форму математически
    # невозможной.
    строф = max(1, min(строф, _КОРОТКАЯ_ВЫДАЧА_МАКС // размер_строфы))
    return строф * размер_строфы, строф
...
def _run(lines, knobs: dict, corpus, nl_fragments: list | None = None, rhyme: str = "none",
    ...
    # Клаузула — Раунд 44, откалибрована по референсным референсным текстам
    ...
    # ТЕМА ВЫРЕЗАНА ЦЕЛИКОМ (2026-08-29, требование: «вообще тему стоит
    ...
    запрошено_строк = int(knobs["shortlist"])
    # До определения пути держим исходный запрос: без индекса ниже нет
    # сборщика строф, и запасная ветка не имеет права обещать округлённую
    # выдачу. На обычном индексном пути эти значения заменятся сразу после
    # `_прямая_тяга`.
    shortlist_к_сборке = запрошено_строк
    n_blocks = max(1, запрошено_строк // stanza_size)
    ...
    # ФОРМАЛЬНАЯ ВЫДАЧА — ТОЛЬКО ЦЕЛЫМИ СТРОФАМИ. До этого `size` мог быть
    # 40 для формы из 14 строк: две строфы собирались, а последние 12 строк
    # становились обречённой попыткой. Маленький запрос вроде 4 для формы из
    # 8 строк давал обратную поломку — ноль, хотя одна строфа возможна.
    # Нормализация живёт только здесь: это единственное место, которое может
    # подтвердить полную строфу и вернуть её границы наружу.
    if _прямая_тяга:
        shortlist_к_сборке, n_blocks = _размер_полных_строф(
            запрошено_строк, stanza_size)
        if shortlist_к_сборке != запрошено_строк:
            knobs = dict(knobs)
            knobs["shortlist"] = shortlist_к_сборке
    # Расчёт после нормализации: для формы 14 строк запрос 40 превращён в 42,
    # и резерв отбора должен быть рассчитан от того же числа, которое реально
    # собирается. Иначе разные ветки одного запроса жили бы с разными лимитами.
    ...
    if _idx is not None:
        ...
        nl_survivors, n_nl_survived, forced_candidates, ступени = nlindex.select(
            ...
            # ПОДРЕЗКА ПРИ ОТБОРЕ (2026-09-01, требование: «мне кнопка не
            # нужна, управляемая вручную»). Движок сам укорачивает длинную
            # строку до вилки слогов, отрезая голову, и сам же проверяет, что
            # подрезанная строка не нарушила ворота. Ворота передаём те, что
            # стоят у человека: `сдвиг` проверяет ровно их.
            подрезка={"ворота": knobs})
    ...
    # Размер запроса — тоже часть воронки: интерфейс должен отличать
    # «попросили 40» от «ядро реально собирало 42 строки = 3 полные строфы».
    # Это особенно важно для формы из 14 строк и для короткого первого запроса
    # фристайла, который может быть меньше одной строфы.
    if syllable_spec and rhyme != "none":
        ступени.update({
            "заказано_строк": запрошено_строк,
            "к_сборке": int(shortlist_к_сборке),
            "размер_строфы": int(stanza_size),
            "заказано_строф": int(n_blocks),
        })
    ...
    # ОДИН ПУТЬ ВМЕСТО ТРЁХ (2026-08-29, снос по требованию: «тему стоит
    ...
    # < 1) — то есть при обычных обычных настройках (только корпус, 95.3%
    ...
    # ФОРМАЛЬНАЯ СБОРКА — НЕ ОЦЕНКА ПУЛА. `в_парах` выше говорит, сколько
    # строк вообще имеют партнёра, а здесь фиксируем итог взаимного отбора:
    # сколько полных строф реально доехало до ответа. Это число нельзя
    # восстановить из воронки по строкам: одна неудачная связь ломает всю
    # строфу, а запрет повторов связывает между собой все её позиции.
    if ступени and _прямая_тяга:
        собранных_строк = len(shortlist)
        собранных_строф = собранных_строк // stanza_size
        верхняя_оценка = ступени.get("верхняя_оценка_строф")
        ступени.update({
            "собрано_строк": int(собранных_строк),
            "собрано_строф": int(собранных_строф),
            "недособрано_строф": max(0, int(n_blocks - собранных_строф)),
            "неполных_блоков": int(ступени.get("неполных_блоков", 0)),
            "поиск_ограничен": int(ступени.get("поиск_ограничен", 0)),
            "поиск_завершён": bool(ступени.get("поиск_завершён", False)),
            "максимум_доказан": bool(ступени.get("максимум_доказан", False)),
            "узлы_поиска": int(ступени.get("узлы_поиска", 0)),
            "недоступные_ворота": list(ступени.get("недоступные_ворота", ())),
            # `None` = дешёвую оценку пропустили (поле слишком велико), 0 =
            # оценка построена и структурно не оставляет ни одной полной
            # строфы. Не смешиваем «не считали» с доказанным нулём.
            "верхняя_оценка_строф": (None if верхняя_оценка is None
                                      else int(верхняя_оценка)),
        })

    # `forced_notice` остаётся ПУСТЫМ СЛОВАРЁМ, а не исчезает: форма ответа
    # /api/generate не меняется ради сноса — старый бандл в браузере
    ...
    if ступени and rhyme != "none":
        ...
        c2: dict = {}
        c1: dict = {}
        ключи_строк = [r.get("_рифмо_ключ") or r.get("rhyme")
                       for r in nl_survivors
                       if r.get("_рифмо_ключ") or r.get("rhyme")]
    ...
    for r in shortlist:
        ...
        r.pop("_рифмо_ключ", None)                    # служебный ключ уже использован в `в_парах`
...
def _rhymes(r1: dict, r2: dict, ярусы: int = 3) -> bool:
    """Рифмуются ли две строки при данной МАСКЕ ярусов (2026-08-28, требование:
...
# НАДГРОБИЕ 2026-09-02: `_rhyme_scheme_groups` И `_nl_slot_plan`.
#
# Обе обслуживали СБОРЩИК СТРОФЫ (`_select_with_rhyme`, 884 строки), вырезанный
# 2026-08-29: первая разбирала схему «абаб» в группы позиций, вторая
# раскладывала квоту корпусных строк по позициям заранее, чтобы рифма не
# ломалась вставкой задним числом. Прямая тяга держит и то и другое сама
# (`nlindex.тянуть_строфы`), и с 2026-08-29 обеих не звал НИКТО: проверено
# обходом дерева разбором, а не текстом, — ноль чтений во всём репозитории,
# включая тесты.
#
# Числились они при этом УЖЕ ПОХОРОНЕННЫМИ: надгробие сборщика ниже называло
# их среди ушедших, а они лежали здесь живым кодом. Надгробие, которое врёт в
# эту сторону, хуже отсутствующего: следующий читатель ищет и не находит того,
# что у него под руками.
...
# НАДГРОБИЕ 2026-09-02: `ПОЛОСА = 12` — ширина полосы разброса при раннем
# выходе сборщика строфы. Сборщик вырезан 2026-08-29, ранний выход вместе с
# ним; читателей у константы с того дня ноль. Сам приём жив и живёт теперь в
# прямой тяге (`nlindex.тянуть_строфы`), со своим числом и своим обоснованием
# на месте.
...
# НАДГРОБИЕ 2026-09-02: `_разброс_семя()`. Отдавала семя для разведения равных
# кандидатов сборщику строфы; сборщик вырезан 2026-08-29, и читать её стало
# некому. Само ЗАКРЕПЛЕНИЕ семени осталось и живее всех живых — на нём стоят
# десять мест, включая `tests/conftest.py`: тяга спрашивает `_РАЗБРОС_СЕМЯ`
# напрямую.
...
# короткое замыкание стояло в ветке СМЕСИ: при обычных обычных настройках
# строфа собиралась колонками, а потом сборщик пересобирал её заново.
#
# ВМЕСТЕ С НИМ УШЛИ: `has_distinct_bucket_partner` и `bucket` — 2026-08-29,
# сразу; `_rhyme_scheme_groups`, `_nl_slot_plan`, `_разброс_семя` и `ПОЛОСА` —
# 2026-09-02, надгробия у каждого на своём месте выше.
#
# А ЭТОТ СПИСОК ВРАЛ В ОБЕ СТОРОНЫ, и это стоит помнить дороже самого списка
# (найдено разбором 2026-09-02). Он хоронил четверых, которые остались лежать
# живым кодом в горячем файле, и ещё двоих, которые ЖИВЫ и сегодня:
#
#   · `_rhymes` — правило пары. Оно не «переехало в `nlindex.ярус_пары`», а
#     стоит здесь же (строка с `def _rhymes`); его читает
#     `tests/test_рифма_ярусы.py`, и это его единственный читатель;
#   · `закрепить_разброс` — десять читателей, включая `tests/conftest.py`.
#
# Мораль ровно та, что записана в PLAN.md после прошлого такого случая:
# прежде чем звать что-то мёртвым, надо НАЙТИ читателей, а не не найти их.
# Список имён, написанный по памяти в момент сноса, — не проверка.
