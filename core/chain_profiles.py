...
# сохранил». Поэтому здесь лежат СЛЕПКИ: каркас и крутилки каждого звена
...
# clean.py, никаких белых списков. Мигрировать было нечего — на диске владельца
...
# три сохранённых пресета пайплайна есть, почему они не отображаются». И он
# прав, а я нет.
#
# До этого раунда пресеты жили константой ВО ФРОНТЕ и полкой не были: серия
# ищет цепочку по имени через `by_name`, там их не было, и владелец упирался в
# стену — три готовых пайплайна на экране и пустой список в серии. Я defended
# это как замысел («заготовка не решение»), но это была не архитектура, а
...
def builtin() -> list[dict]:
    ...
    собирал бы владелец руками. Прогоняются через тот же валидатор, что и
