...
#
# ПОЧЕМУ СНЯТО. Решено 2026-08-29: «тему стоит вырезать
...
def knobs(raw: dict | None) -> dict:
    ...
    # требование: «вырезать всё вместе с темой».
    #
    ...
    return {
        ...
        # Требование — «диапазон с 5 по 10 процентов +
...
KNOB_GATES = {
    ...
    # Генератор вырезан целиком — решение: «генератор — вырезать», —
    ...
    # КЛАУЗУЛА — МАСКА КОНЦОВОК (2026-08-28, требование: «может я хочу только
    ...
    # ). 0 — неважно, 1 — только строки, где рифмуются два РАЗНЫХ
    ...
    # ПОЗИЦИЯ РИФМЫ — МАСКА СОРТОВ (2026-08-30, требование: «рифма так-то
    ...
    # ПЕРЕКЛИЧКА СОГЛАСНЫХ (2026-08-30, требование: «не только гласные,
...
KNOB_OPINIONS = {
    # ЯРУСЫ РИФМЫ — БИТОВАЯ МАСКА, А НЕ ПОЛЗУНОК (2026-08-28, требование:
    ...
    # ручка осталась бы ползунком без шкалы. Решено 2026-08-29:
    ...
    # 1.0 → +0.292, P < 0.0001), но управлял тем, что выбрасывается: он
...
def knob_params(raw) -> dict:
    ...
    if "Клаузула" in raw:
        try:
            ...
            # рабочем складе клаузула стоит «любая», а редкий старый 3 маска
...
def knobs_from_profile(profile: dict | None) -> dict:
    ...
    return knobs({
        ...
        # грамматических строк вырезан целиком (решение: «генератор —
...
# решением от 2026-08-18 («строфа единственным режимом»), и это его
# решение, а не замер.
#
