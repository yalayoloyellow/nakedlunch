...
def knobs(raw: dict | None) -> dict:
    ...
    # 0.83 — прежнее положение пользователя (0.35 по старой односторонней шкале)
...
KNOB_OPINIONS = {
    ...
    # ДВУСТОРОННЯЯ (Раунд 58, пользователь): 0 — язык максимально затёртый
    ...
    # Дефолт 0.83 — это ПРЕЖНЕЕ поведение пользователя, пересчитанное в новую
...
#
# Требование (2026-08-03): одним нажатием выбирать готовое решение целиком.. Ссылка на полку это обещание нарушает: подкрутил профиль ради
...
def chain_link(raw, index: int = 0) -> dict | None:
    ...
    молча подставить «какую-нибудь» строфу значит спорить с пользователем.
...
# ПРЕДЕЛОВ ЗДЕСЬ НЕТ, и это решение. требование (2026-08-04): убрать предел.. Он прав: любое число, которое я бы здесь поставил, было бы
...
def series_link(raw) -> dict | None:
    ...
    # Снизу единица: звено, которое не даёт ни одного текста, пользователь потом
...
def pipeline_spec(raw) -> dict:
    ...
    #   (Раунд 48, требование: один вариант вместо десяти вариаций),
    ...
    # `weights` — три веса склейки. требование (2026-08-03): то, что от касания становится хуже, лучше убрать.. Качество и стыки нужны всегда
