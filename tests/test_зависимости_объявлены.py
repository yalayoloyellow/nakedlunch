...
#   · huggingface_hub — нужен только установщику, чтобы один раз положить
#     модель акцентуатора в готовый пакет;
...
НЕ_ТРЕБУЕМ = {
    "huggingface_hub",                 # только загрузка модели при установке
...
def test_акцентуатор_именно_объявлен():
    """Динамический импорт из сдвига не увидит общий обход AST."""
    есть = объявлено()
    assert {"ruaccent", "onnxruntime", "transformers"} <= есть, (
        "акцентуатор нужен звуковым воротам после сдвига строки; без него "
        "свежая установка падает только при включённой настройке")
