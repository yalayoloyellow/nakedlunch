...
#
# Как очередь серия работала и раньше. Ломалась она там, где владелец забирает
...
#      владельца, — при том что в коде было написано, что он защищён.
#
...
def test_nepustoy_koren_seriy_ne_vysypaetsya(хранилище):
    ...
    названия высыпались поверх собственных текстов владельца.

    Отказ НАЗЫВАЕТ, что внутри: «не удаляется» без причины владелец читает как
...
def test_pustoy_koren_seriy_udalyaetsya(хранилище):
    """ПОЧИНКА В ТОТ ЖЕ ДЕНЬ. Сначала я запретил корень совсем — и владелец
    сказал «не могу папки удалять»: у него это была единственная папка, к тому
...
def test_vnutri_kornya_papki_udalyayutsya(хранилище):
    ...
    и запрещать их значило бы отобрать у владельца уборку."""
...
def test_deystviya_lista_vidny_bez_navedeniya():
    """Владелец: «у меня кнопок удаления папок и листов нет вообще».
