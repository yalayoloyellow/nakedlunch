...
def test_ambiguous_forms_collapse_by_lemma_set():
    ...
    «утра, утром» и «спать, спал» как разные слова — ровно жалоба пользователя
    2026-08-02."""
...
def test_цена_сшивки_на_живых_данных(real_index):
    """Цена названа: 100 мс на слово. До волны С2 худший полный
    ...
    # И ТЕКСТОВЫЙ БЛОБ — по той же причине. Подписи собираются через
    # `idx.text()`, то есть чтением из memmap; у живого пользователя эти
    # страницы горячие всегда, потому что сервер на прогреве строит по ним
    # карту `text_ids` (nlindex.прогреть). До 2026-09-01 их случайно грел скан
    # целостности внутри `whole_mask`; скан стал колонкой, грелка исчезла, и
    # тест начал мерить чтение с диска под нагрузкой набора — 174 мс вместо 60.
    # Один байт на страницу поднимает блоб целиком за миллисекунды.
    np.asarray(nlindex.load().blob[::4096]).sum()
    ...
    try:
        ...
        #
        # ЛУЧШЕЕ ИЗ ТРЁХ, А НЕ ОДИН ЗАМЕР (2026-09-02). Здесь стоял один прогон
        # на слово и потолок 100 мс — вдвое от исторических 52. Запаса не было
        # вовсе, и сторож краснел от НАГРУЗКИ МАШИНЫ, а не от кода: в полном
        # наборе выходило 174, 276, 485 мс, в одиночку на той же сборке — 53.
        # Красный набор, который то краснеет, то зеленеет сам по себе, хуже
        # отсутствующего сторожа: ему перестают верить.
        #
        # Лучшее из трёх меряет то, что тест и хочет измерить, — ЦЕНУ РАБОТЫ, а
        # не то, сколько соседей крутилось на машине в ту секунду. Медленный
        # код не спрячется: он медленный во всех трёх.
        #
        # Потолок поднят до 250 мс — впятеро от исторического худшего, ровно
        # как велит правило проекта о щедрых порогах («ловят возврат прежнего
        # поведения, а не колебания машины», PLAN.md, «Риски»). Возврат к
        # прежней цене — это секунды на слово, и 250 мс их поймает с запасом.
        худшее = 0.0
        for w in ("над", "пор", "он", "дом", "мороз", "свет", "дорога"):
            ...
            for _ in range(3):
                ...
                замеры.append((time.perf_counter() - t) * 1000)
                assert items, w
            худшее = max(худшее, min(замеры))
        assert худшее < 250, f"худшее слово {худшее:.0f} мс при потолке 250"
    finally:
