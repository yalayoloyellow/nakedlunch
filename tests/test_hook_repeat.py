...
#
# Требование (2026-08-04): хук — тип строфы, подразумевающий повтор; механизм обязан уметь
# хуки и хуковые припевы, а не бороться с повтором..
#
...
def test_pripev_ne_stoit_generatsii():
    ...
    входит: припев не должен стоить пользователю второго прогона генерации."""
    s = спека([{"spec": каркас()}, {"title": "припев", "repeat_of": 0}])
    links = pipeline.resolve_chain(s["chain"], s["knobs"])
    звали = []
...
# что указано: борьба с повторяющимся здесь неуместна..
#
...
# формы строф: три сохранённых пресета., и ответ был — потому что полкой они не были.
...
def test_vse_tri_vstroennye_sobralis():
    ...
    молча оставило бы пользователя без встроенных цепочек."""
    в = встроенные()
...
def test_seriya_nahodit_vstroennuyu_po_imeni():
    ...
    фронте. Отчёт: три сохранённых пресета пайплайна не отображались.."""
