...
def test_повторная_кнопка_во_время_показа_честно_говорит_занято():
    """Даже у уже не живого окна действующая строфа означает «занято»,
    а не ложное «окно офлайн»."""
    sys.path.insert(0, str(КОРЕНЬ / "api"))
    import server
    ...
    # ticket — уже доказательство захваченной строфы. Состояние окна истекает
    # явно, а не через недетерминированную паузу планировщика.
    with пульт._условие:
        пульт._окно_до = 0.0
