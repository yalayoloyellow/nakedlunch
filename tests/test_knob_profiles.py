# nakedlunch — вторая полка: ПРОФИЛИ НАСТРОЕК (Раунд 50, владелец: «можно
# делать строфы, и можно сделать профиль настроек — как расположены крутилки;
...
#
# Проверяется РЕЗУЛЬТАТ, а не наличие ключа: владелец на этом уже обжигался
# («рифмовка работала по наличию ключа, а не по совпадению»), поэтому каждый
...
def test_профиль_без_имени_это_не_профиль():
    """None, а не исключение: одна битая строка в файле, который владелец
...
def test_классика_не_хранит_мнений():
    """Владелец: «всё, что не работает, лучше убрать вовсе». Профиль в режиме
...
def test_дефолт_мата_не_режет_мат():
    ...
    мата», хотя в референсах владельца мата 17–21%. Ядро всегда считало
...
def test_свой_профиль_перекрывает_встроенный(свой_каталог):
    ...
    тем же именем — сознательный оверрайд владельца."""
