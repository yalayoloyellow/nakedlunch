...
#
# Владелец 2026-08-05: «жму отключить книгу и непонятно вообще, отключится или
# нет… иногда после нажатия отключить книгу она не отключается».
#
# Причина была одна на обе жалобы: `toggle_active` звал `_save(full=True)`, то
# есть перезапись ВСЕГО state.json ради одного булева. На его корпусе это
# 549 МБ — десятки секунд, за которые в интерфейсе не менялось ничего, и он
# жал второй раз, переключая книгу обратно.
#
...
def test_tokeny_schitayutsya_lenivo(tmp_path):
    ...
    состава активных книг: на корпусе владельца это 2.87 млн вызовов регулярки
...
def test_otkaz_servera_vozvrashchaet_tochku():
    """Сервер отказал — точка возвращается на место, и владелец слышит почему.
...
def test_vtoroy_klik_vo_vremya_pohoda_ne_schitaetsya():
    ...
    переключалась бы обратно, и это ровно то, что владелец видел как «иногда
    не отключается»."""
    out = node(ОСНОВА + """
    const c = макет();
    c.state.srcBusy = { a: 1 };
...
def test_pachka_knig_pishet_korpus_odin_raz(tmp_path):
    ...
    state.json на корпусе владельца весит 549 МБ. `add_corpus` писал его сам,
