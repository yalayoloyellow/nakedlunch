...
#
# Пользователь 2026-08-05: «жму отключить книгу и непонятно вообще, отключится или
...
def test_vtoroy_klik_vo_vremya_pohoda_ne_schitaetsya():
    ...
    переключалась бы обратно, и это ровно то, что пользователь видел как «иногда
    не отключается»."""
    out = node(ОСНОВА + """
    const c = макет();
    c.state.srcBusy = { a: 1 };
