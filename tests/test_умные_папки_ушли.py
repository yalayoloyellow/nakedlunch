...
#
# ПОЧЕМУ ЭТО НЕ КОСМЕТИКА. Файл настроек — то, что  открывает руками, и
...
#   2. старый живой файл с этим ключом читается, а не падает: чтение молча
...
# Файл, какой может лежать на диске прямо сейчас: мёртвый ключ
# вперемешку с живыми.
...
def test_chtenie_starogo_fayla_perezhivaetsya(файл):
    """Ключ лежит на диске, и чтение обязано молча его выбросить.
...
def test_pervaya_zhe_zapis_ubiraet_klyuch_s_diska(файл):
    """Иначе мёртвый ключ жил бы в файле пользователя вечно — как «Разнообразие»
