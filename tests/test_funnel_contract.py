...
# ---- ступени: то, чего в воронке НЕ БЫЛО (Раунд 62) ------------------------
#
# Замер 2026-08-13 нашёл в воронке три лжи разом:
#   · крупнейшей ступени каскада не было вовсе. Между `nl_survived` и
#     `shortlist` читалось «из 1.5 млн выбрано 8», а на деле —
#     «из 1 564 599 СЛУЧАЙНО взято 300, и уже из 300 схема собрала 8»;
#   · `nl_survived` снимался ПОСЛЕ отсева повторов темы и с темой врал на
#     3 528 строк: они прошли ворота честно, а в счётчике их не было;
#   · четыре счётчика из десяти описывают грамматическую ветку, которая при
#     «Источники = корпус» не запускается вовсе, и нули читались как
#     «отфильтровано подчистую».
#
# Сторож проверяет ИСХОД: цепочка обязана быть настоящей цепочкой.

def test_voronka_govorit_zapuskalas_li_grammatika(словарь):
    """Нули грамматической ветки обязаны отличаться от «ступеней не было».
    Без этого поля воронка врала о причине: «Источники = корпус» показывал
    четыре нуля, и это читалось как «всё отфильтровано»."""
    только_корпус = filters.run([], clean.knobs({"shortlist": 4, "real_text": 1.0}),
                                Corpus(), nl_fragments=list(ФРАГМЕНТЫ), rhyme="аабб")
    assert только_корпус["funnel"]["gen_ran"] is False
    assert только_корпус["funnel"]["generated"] == 0

    import generate
    строки = generate.generate([], n=200, seed=1)
    смешанный = filters.run(строки, clean.knobs({"shortlist": 4, "real_text": 0.5}),
                            Corpus(), nl_fragments=list(ФРАГМЕНТЫ), rhyme="аабб")
    assert смешанный["funnel"]["gen_ran"] is True


def test_obe_vetki_dayut_stupeni_odnoy_formy(словарь):
    """Форма ответа одна — значит и ступени одни. Разная форма означала бы «на
    классике посмотреть нельзя» без единого слова об этом."""
    алго = filters.run([], clean.knobs({"shortlist": 4, "real_text": 1.0}),
                       Corpus(), nl_fragments=list(ФРАГМЕНТЫ), rhyme="аабб")
    класс = filters.run([], clean.knobs({"shortlist": 4, "real_text": 1.0, "classic": 1.0}),
                        Corpus(), nl_fragments=list(ФРАГМЕНТЫ), rhyme="аабб")
    for r in (алго, класс):
        assert "ступени" in r["funnel"] and "gen_ran" in r["funnel"]
    if алго["funnel"]["ступени"] and класс["funnel"]["ступени"]:
        assert set(алго["funnel"]["ступени"]) == set(класс["funnel"]["ступени"])
