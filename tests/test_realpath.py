...
def test_форма_и_рифма_обе_жёсткие(monkeypatch):
    ...
    Не нашлось строки, годной по обоим, — неполный блок целиком не выходит.

    Два сценария: (1) годная пара есть — обе строки в вилке и в рифме;
    (2) годной пары нет — строфа не выходит вовсе, а не заполняется строкой не
    той длины или не в рифму. Тихая подстановка «почти подходящей» строки —
    ровно та ложь, которой тут не место."""
    ...
    for семя in range(10):
        ...
        assert строфа == [], (
            "партнёра нужной длины не было — тяга обязана отбросить "
            f"неполную строфу, а не показать её кусок: {строфа}")
