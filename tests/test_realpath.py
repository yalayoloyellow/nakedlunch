...
def test_forced_word_hard_guarantee():
    """!слово (PLAN.md 0.2b, реализовано 2026-07-17) — требование: `!слово` — обязательный показ этого слова в одной из строк, если оно есть в
    базе.. В отличие от обычной темы (0.2a: ≈1 на строфу,
...
def test_theme_anchor_present_on_topic_and_not_a_duplicate():
    """Требование (2026-07-18): при диссонансе обязана быть одна строка точно в тему, чтобы с
    ней диссонировать; созвучные не должны быть идентичны основной.. Сценарий:
...
def test_all_grammar_templates_actually_produce_output():
    ...
    Позже требование: больше разнообразия, в том числе неровные обрезки, как в кат-апе. — добавлены 6 «рваных» шаблонов
...
def test_stanza_syllable_length_prefers_but_never_beats_rhyme():
    """Конструктор строф (PLAN.md 0.7) — требование: выбор минимального и максимального числа слогов в каждой строке., и на прямой вопрос
...
def test_splice_fills_a_too_short_slot_with_a_rhyming_tail():
    """Идея требование (2026-07-19): не хватает слогов — добавлять строку, подходящую по слогам и
    рифме.. Позиция 2 схемы "аа" может получить нужную длину только
...
def test_trim_shortens_an_overlong_candidate_without_touching_the_rhyme():
    """Идея требование (2026-07-19): слишком длинные строки укорачивать, отрезая наименее значимые
    слова.. Единственный кандидат позиции 2 (25 слогов)
