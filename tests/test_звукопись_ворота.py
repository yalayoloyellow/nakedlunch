# ЗВУКОПИСЬ В ОТБОРЕ: ворота переклички и полосы плотности (2026-08-30).
#
# Измеритель проверен отдельно (test_звукопись.py). Здесь — стык: ворота
# действительно сужают пул, полосы работают как у редкости, а индекс БЕЗ этих
# колонок не врёт и не режет.
#
# КОЛОНКИ ПОДКЛАДЫВАЮТСЯ ЖИВОМУ ИНДЕКСУ, а не строится игрушечный: ворота
# проходят через чёрный список и отпечаток корпуса, которым нужен настоящий
# индекс целиком. Подложка снимается в `finally` — иначе она утекла бы в
# соседние тесты через общий кэш загрузчика.
import sys
from pathlib import Path

import numpy as np
import pytest

sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "core"))
import nlindex  # noqa: E402
import звукопись as з  # noqa: E402


@pytest.fixture()
def стенд():
    idx = nlindex.load()
    if idx is None:
        pytest.skip("нет живого индекса")
    было = (getattr(idx, "echo", None), getattr(idx, "dens", None))
    порог = int(з.БАЛЛ_МИН * 10)
    n = idx.n
    # перекличка: у каждой пятой строки — «есть», у остальных ноль
    echo = np.zeros(n, dtype=np.uint8)
    echo[::5] = порог + 3
    # плотность: ровный процентиль 0..99 по кругу, у каждой седьмой шкалы нет
    dens = (np.arange(n) % 100).astype(np.float32)
    dens[::7] = -1.0
    idx.echo, idx.dens = echo, dens
    try:
        yield idx, echo, dens
    finally:
        idx.echo, idx.dens = было
        nlindex.забыть_таблицу()


def test_ворота_переклички_сужают_пул(стенд):
    idx, echo, _ = стенд
    без = idx.gate_mask(False)
    с = idx.gate_mask(False, перекличка=1)
    assert 0 < с.sum() < без.sum()
    # ровно те строки, у которых балл дотянул до порога, и ни одной сверх
    assert not (с & (echo < з.БАЛЛ_МИН * 10)).any()


def test_полосы_плотности_как_у_редкости(стенд):
    idx, _, dens = стенд
    верх = idx.gate_mask(False, плотность=[(90, 100)])
    низ = idx.gate_mask(False, плотность=[(0, 10)])
    две = idx.gate_mask(False, плотность=[(0, 10), (90, 100)])
    assert верх.sum() and низ.sum()
    assert not (верх & низ).any(), "полосы не должны пересекаться"
    assert две.sum() == верх.sum() + низ.sum(), "несмежные полосы обязаны складываться"
    # строка без шкалы (−1) не попадает даже в полосу «вся шкала» — как у редкости
    любая = idx.gate_mask(False, плотность=[(0, 100)])
    assert not любая[np.flatnonzero(dens < 0)].any()


def test_классика_подчиняется_воротам_звукописи(стенд):
    """Ручка, которую экран показывает в классике, обязана в классике и
    действовать: иначе это контрол-обманка (то же правило, что у мата)."""

# НАДГРОБИЕ 2026-09-02: ЭТОТ СТОРОЖ ОХРАНЯЛ ОТМЕНЁННЫЙ ДОГОВОР.
#
# Он требовал, чтобы классика ПОДЧИНЯЛАСЬ этим воротам, и обоснование звучало
# правдоподобно: «ручка, которую экран показывает в классике, обязана в
# классике и действовать, иначе это контрол-обманка». Правдоподобно — и мимо
# режима: клаузула, звукопись и полосы редкости это про ФОРМУ строки, а
# классика по определению форму не судит.
#
# Требование 2026-09-02: «классика вообще все настройки отключает и
# просто даёт равную выдачу». Оставлены только мат, чёрный список, история и
# активный пул — сказанное вслух про СОДЕРЖАНИЕ. Разбор и замер — в надгробии
# внутри `nlindex.select_light`; на экране снятые ручки гаснут подписью
# «классика этого не читает» (`render.gen.jsx`, `классикаНеЧитает`).
#
# Сторож перевёрнут: теперь он держит НОВЫЙ договор — что ворота НЕ действуют.
    import inspect
    подпись = inspect.signature(nlindex.select_light).parameters
    assert "перекличка" not in подпись, "классика снова читает звукопись"
    assert "плотность" not in подпись


def test_воронка_называет_ступень(стенд):
    """Цена ручки — числом и по имени: воронка обязана назвать, КТО закрыл
    ворота, иначе «дожило» безымянно."""
    idx, _, _ = стенд
    д = nlindex.воронка(перекличка=1, плотность=[(90, 100)])
    имена = [ш["шаг"] for ш in д["ступени"]]
    assert "перекличка согласных" in имена
    assert any(ш.startswith("плотность звука") for ш in имена)


def test_индекс_без_колонок_не_пропускает_жёсткие_ворота():
    """Отсутствующая колонка не может превратить жёсткую ручку в no-op.

    Старое поведение оставляло весь пул и только отдельно предупреждало UI.
    Это создавало слепую зону: пользователь включил ворота, а выдача шла без
    них. Теперь запрос честно даёт ноль, а форма называет каждую недоступную
    ось — перепечь индекс можно до выдачи, а не гадать по результату."""
    idx = nlindex.load()
    if idx is None:
        pytest.skip("нет живого индекса")
    было = (getattr(idx, "echo", None), getattr(idx, "dens", None),
            getattr(idx, "rare_word", None), getattr(idx, "rare_pair", None))
    idx.echo = idx.dens = None
    idx.rare_word = idx.rare_pair = None
    try:
        nlindex.забыть_таблицу()
        с = idx.gate_mask(False, перекличка=1, плотность=[(90, 100)])
        assert int(с.sum()) == 0, "без колонок жёсткие ворота не должны пропускать пул"
        форма = nlindex.форма_пула(idx, pool_mask=np.ones(idx.n, dtype=bool),
                                   hidden_mask=np.zeros(idx.n, dtype=bool))
        assert форма["звукопись"] is False
        assert форма["перекличка"] is False
        assert форма["плотность_звука"] is False
        assert форма["редкость_слов"] is False
        assert форма["редкость_сочетаний"] is False
    finally:
        idx.echo, idx.dens, idx.rare_word, idx.rare_pair = было
        nlindex.забыть_таблицу()


def test_воронка_не_скрывает_отсутствующую_колонку():
    """Карта ступеней не превращает недоступные ворота в невидимый no-op."""
    idx = nlindex.load()
    if idx is None:
        pytest.skip("нет живого индекса")
    было = (getattr(idx, "echo", None), getattr(idx, "dens", None))
    idx.echo = idx.dens = None
    try:
        nlindex.забыть_таблицу()
        карта = nlindex.воронка(перекличка=1, плотность=[(90, 100)])
        assert карта["недоступные_ворота"] == ["перекличка", "плотность звука"]
        assert карта["ступени"][-1]["дожило"] == 0
        имена = [ш["шаг"] for ш in карта["ступени"]]
        assert "перекличка согласных недоступна" in имена
        assert "плотность звука недоступна" in имена
    finally:
        idx.echo, idx.dens = было
        nlindex.забыть_таблицу()
