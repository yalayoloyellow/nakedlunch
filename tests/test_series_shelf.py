...
def test_tema_neobyazatelna():
    ...
    половину того, чем владелец пользуется."""
...
def test_predelov_razmaha_net():
    """Владелец 2026-08-04: «зачем вообще предел, убери». Любое число здесь
...
def test_bitaya_zapis_ne_unosit_vsyu_polku(полка):
    """Одна кривая строка в файле не должна стоить владельцу всех серий."""
...
def test_ocenka_schitaet_teksty_i_sekundy():
    """Владелец хотел «тысячи претендентов на трек» — это 17 суток на сто
