...
def api_status():
    ...
    items = [x for x in [_nl_store_status(), _import_status(), _nl_rhyme_status(),
                         _nl_index_status(), _устарелость_статус()]
             if x is not None]
...
def api_nl_clean():
    ...
    if err:
        return err
    сухой = bool((request.get_json(force=True, silent=True) or {}).get("сухой"))
    with nlindex.ЗАМОК:
        итог = _nl().почистить(применить=not сухой)
    if сухой:
        return {"ok": True, "сухой": True, **итог, "пересчёт": False}
    nlindex.forget_pool()
    ...
    почищено = _выбросить_из_кэша(_nl().get_active_pool())
    пошло = _nl_rhyme_ensure_running()
    return {"ok": True, "сухой": False, **итог,
            "из_кэша_убрано": почищено, "пересчёт": bool(пошло)}
