...
def _прогрев_статус() -> dict | None:
    ...
    ЗАЧЕМ. Требование владельца (2026-08-21): «лучше мне показывать загрузочный
...
# владельца — «добавить книгу» — гасил единственный режим программы.
#
...
# значка в доке нет, программы для владельца больше не существует — а ребёнок
...
#      от той, что владелец запустил из своего терминала руками, — а её
...
def _проследить_за_сборкой(proc) -> None:
    ...
    def смотреть():
        ...
        # памяти» — то есть врал бы владельцу про его же закрытие окна, и врал
...
# «СКОЛЬКО ОСТАЛОСЬ», А НЕ «СКОЛЬКО ПРОШЛО» (2026-08-28, владелец: «зачем
...
# словарный слой в `/api/word/suggest`; роут владелец решил оставить.
...
#
# ЧТО СЛУЧИЛОСЬ. Приложение владельца, поднятое в 04:40, в 08:27 сохранило
...
#
# ЧЕМ ЛЕЧИМ. Не версией в самом складе — лишнее поле в файле владельца это
...
def api_nl_index_run():
    ...
    то есть при работе над кодом, а не в обычной жизни владельца."""
    err = _nl_guard()
    if err:
        return err
    if _NL_INDEX_PROC is not None and _NL_INDEX_PROC.poll() is None:
...
# когда её меняют. У владельца нет положения, из которого он захотел бы
...
def api_nl_clean():
    ...
    длинная, и решать, когда отдать под неё машину, — дело владельца.
...
# живая проверка владельцем кончилась так: «просто всплывает надпись „посчитаю“
...
# слово» в ленте; кнопка снята в тот же день по слову владельца («мне кнопка не
...
def api_settings_post():
    ...
    to_save = {
        ...
        # владельца, 2026-08-28. Ключ убран И из settings._ALLOWED: фильтра
...
# оставлены по решению владельца.
#
