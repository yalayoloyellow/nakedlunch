...
def _прогрев() -> None:
    ...
    def история():
        """Словарь «текст → номер» — ТОЛЬКО если история непустая.

        Он стоит 5.4 с и нужен ровно для перевода показанных строк в номера.
        Пустая история его не трогает (`nlindex.mask_of`), а активный пул с
        2026-09-03 берётся из колонки `src`. Греем последним и по условию:
        постройка словаря держит GIL и душит запросы, а ждать её незачем."""
        idx = nlindex.load()
        if idx is None or not CORPUS.hidden_set():
            return
