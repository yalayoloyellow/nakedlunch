...
# НАДГРОБИЕ: ПОЛЕ `thesaurus` В ОТВЕТЕ `/api/state` И ВОРОТА КАЧЕСТВА
# `_ANT_GATE_PASSED` ВЫРЕЗАНЫ 2026-08-18 — их некому было читать. Сам
# попап по слову, его словари и `core/wordsuggest.py` вырезаны следом:
# разбор в надгробии у бывшего роута, ниже по файлу.
...
def _устарел_процесс() -> str:
    ...
    try:
        ...
        # ЭТА ПРОВЕРКА И ЕСТЬ ГОЛОС О БЕДЕ. Проглотив её падение, окно молчало
        # бы дважды: и о том, что кэш устарел, и о том, что проверить не вышло.
        журнал.запись("сервер", f"проверка свежести кэша не отработала: {e}", "внимание")
        return "проверить свежесть кэша ударений не вышло — смотри журнал"
    return ""
...
# НАДГРОБИЕ: ПОПАП ПО СЛОВУ (POST /api/word/suggest) ВЫРЕЗАН.
#
# Экрана у роута не было с 2026-08-18: `methods.слово.js` удалён, обёртка
# `wordSuggest` во фронте вызывающих не имела. Роут держал три словаря —
# `core/data/rhyme_index.json`, `core/data/navec.tar`, `data/thesaurus.json`,
# вместе 125 МБ на диске — и платил за них на старте: `embeddings.warm_caches()`
# 0.3 с, а первый вызов роута ещё 2.28 с и 751 МБ памяти до конца сессии.
#
# Вместе с роутом ушли `core/wordsuggest.py`, `core/embeddings.py`, их печки
# `tools/build_rhyme_index.py`, `tools/build_thesaurus.py`,
# `tools/скачать_векторы.py` и колонка `lem2navec` в печке индекса.
# `RULES` не менялся — испечённый индекс остаётся своим, перепечка не нужна.
