...
#
# ЧИНИМ ЗДЕСЬ, А НЕ ТАМ, ГДЕ БОЛИТ, И ЭТО НАЗВАНО ПРЯМО. Правильное место для
# починки — сам `nlindex.запрет` (должен брать индекс аргументом, как все
# соседи) и кэш в `filters`, который обязан знать про смену индекса. Оба файла
# в этом раунде за другими агентами, поэтому лечение стоит на стороне
# сервера — он и так единственный, кто перезагрузку затевает. Долг назван в
# отчёте.
#
# ДВЕ ЧАСТИ ЛЕЧЕНИЯ:
#   1. перезагрузка идёт ОДНИМ местом (`_перечитать_индекс`), и оно сбрасывает
#      ВСЕ производные виды разом — включая чужой кэш в filters;
#   2. пока запрос смотрит на индекс, перезагрузка ждёт. Иначе остаётся гонка:
#      сборка кончается ровно посреди `filters.run`, и запрос, начатый со
#      старыми колонками, доходит до `запрет()` уже с новыми.
#
# ЦЕНА ЗАМКА. Незанятый `RLock` — 0.109 мкс на пару acquire/release (замер:
# 10⁶ пар, 0.109 с). Против 185 мс самого прогона это 0.00006% запроса.
# Занятым он бывает только в конце перепечки — тогда одна генерация подождёт
# другую. Плата за то, что отказа нет вовсе.
_ИНДЕКС_ЗАМОК = threading.RLock()


def _перечитать_индекс() -> None:
    """Перечитать колонки с диска и забыть ВСЁ, что от них производно.

    Единственная дверь к перезагрузке индекса в этом файле. Второй такой двери
    быть не должно: разъехавшиеся списки «что ещё забыть» — уже трижды
    случившаяся в этом проекте болезнь (прогрев в Раундах 51 и 62, формат кэша
    рифм)."""
    with _ИНДЕКС_ЗАМОК:
        # ПОЧЕМУ НЕ `nlindex.reload()` ОДНОЙ СТРОКОЙ. `reload` — это ровно
        # `forget(); warm_background()`, и между ними колонок не загружает
        # никто: их грузит фоновый поток. А `nlindex.load()` не защищён
        # замком — он ставит `_tried = True` ДО того, как построит объект,
        # поэтому второй вызывающий в этот момент получает None и верит ему.
        # Замерено на стенде: сразу после `reload()` `nlindex.load()` отдаёт
        # None, через 0.2 с — индекс. Цена этого — `_перепривязать_историю`,
        # которую зовут следующей строкой: она видит None, честно решает
        # «сверять не с чем» и молча не делает работу, ради которой её и
        # позвали, — то есть после КАЖДОЙ перепечки история переставала
        # сходиться с корпусом.
        #
        # Настоящее место починки — замок внутри `nlindex.load`; файл в этом
        # раунде за другим агентом, долг назван в отчёте. Здесь грузим
        # синхронно, под своим замком, и только потом пускаем фоновый прогрев.
        nlindex.forget()          # забыть колонки, карту «текст → номер», таблицы
        nlindex.load()            # …и сразу поднять новые — в этом же потоке
        nlindex.warm_background()  # карты (9.8 с + 9.2 с) греются фоном, как и раньше
        _forget_pool_mask()       # маска активного пула — она по номерам строк
        # ЧУЖОЙ КЭШ, И ЭТО НЕ ОПИСКА. `filters._index_verdict` держит объект
        # индекса ключом по кэшу рифм; перезагрузка индекса этот ключ не
        # трогает, и без сброса filters вечно отдавал бы прежний объект — ровно
        # то, из-за чего и была пятисотка. Сбрасываем публично признанным
        # способом: присваиванием None, как это делает сам filters.
        filters._index_verdict = None
...
def _nl_index_ensure_running() -> None:
    ...
    _после(_NL_INDEX_PROC, lambda: (_перечитать_индекс(), _перепривязать_историю()))
...
# настоящий, вкладка ВКЛЮЧЕНА.
#
# ВОРОТА ЗАКРЫВАТЬ БОЛЬШЕ НЕЧЕГО, И СКАЗАТЬ ЭТО НАДО ПРЯМО (2026-08-18).
# Здесь стояло «провал будущей пересборки → False, вкладка прячется на фронте
# (render.doc.jsx фильтрует по /api/state.thesaurus.ant)». `render.doc.jsx`
# удалён вместе с редактором документа, попап слова вырезан, вкладки
# «антонимы» не существует, а поле `thesaurus` фронт перестал даже
# спрашивать — сверено `grep -rn "thesaurus" interface/react-app/src/`: три
# попадания, и все три — надгробия и список запрещённых к переносу ключей
# состояния, ни одного чтения. То есть механизма, который этот комментарий
# описывал, на свете нет.
#
# Поле в ответе оставлено, но оставлено НЕ «на всякий случай»: его читает
# `tests/test_wordsuggest.py::test_state_reports_real_thesaurus_layers`, а
# существующие тесты в этом раунде правит другой агент. Снести поле, ворота и
# `wordsuggest.thesaurus_status()` одним куском — работа для того, кто держит
# этот тест; отчёт про это долг называет.
...
def api_state():
    ...
    # синонимов; ant — словарь есть И ворота качества пройдены (константа
    # выше). Читателя у обоих флагов сейчас нет — см. надгробие у ворот.
...
def api_nl_funnel():
    ...
    try:
        ...
        pass
    # Под замком: воронка тоже считает маски по колонкам, и перепечка посреди
    # счёта дала бы ту же несходящуюся пару — см. `_перечитать_индекс`.
    with _ИНДЕКС_ЗАМОК:
...
def api_pool_shape():
    ...
    # Замок ОБНИМАЕТ И ВЫБОР ИНДЕКСА, И СЧЁТ ПО НЕМУ: между `load()` и
    # `форма_пула` укладывается перепечка, и тогда колонки будут от старого
    # индекса, а маска чёрного списка внутри — от нового.
    with _ИНДЕКС_ЗАМОК:
        idx = nlindex.load()
...
def api_generate():
    ...
    # ПОД ЗАМКОМ ИНДЕКСА, ЦЕЛИКОМ. `filters.run` берёт объект индекса в начале
    # и спрашивает у него маски до самого конца; перепечка, доехавшая в
    # середину, оставляла запрос со старыми колонками и новой маской чёрного
    # списка — та самая пятисотка, см. `_перечитать_индекс`.
    with _ИНДЕКС_ЗАМОК:
...
# минуту.
#
# ЧТО ОСТАЛОСЬ БЕЗ ВЫЗЫВАЮЩИХ В `core/nlbridge.py`: `get_config`,
# `set_retention`, `clear_used_for_period` и константа `_RETENTION_DAYS`.
# Не тронуты нарочно — файл в этом раунде за другим агентом; долг назван в
# отчёте.
#
