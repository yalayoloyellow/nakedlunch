...
def _чистка_поток(считать_только: bool) -> None:
    ...
    try:
        ...
        журнал.ошибка(f"чистка корпуса: {e}")
...
def _снимок_склада() -> None:
    ...
    try:
        ...
        журнал.ошибка(f"снимок склада перед чисткой не сделан: {e}")
...
def _после_чистки(итог: dict) -> None:
    ...
    итог["из_кэша_убрано"] = _выбросить_из_кэша(_nl().get_active_pool())
...
def _выбросить_из_кэша(живые_тексты) -> int:
    ...
    if _сборка_сейчас():
        журнал.ошибка("кэш ударений не подчищен: идёт сборка")
        return 0
