...
def _перепривязать_историю() -> None:
    ...
    # ШТАМП СВЕРЯЕТСЯ ДО ПОСТРОЙКИ СЛОВАРЯ (2026-09-03).
    #
    # Здесь `text_ids` строился ПЕРВЫМ — 5.4 секунды и сотни мегабайт, — и
    # только потом `перепривязать` смотрел на штамп и в большинстве случаев
    # отвечал «работы нет». Это главный случай по его же докстринге: штамп
    # совпадает при КАЖДОМ запуске, кроме первого после перепечки.
    #
    # Цена ошибки была не только во времени. Постройка словаря — сплошной
    # питон, она держит GIL, и первая генерация ждала её целиком: замер давал
    # 15 секунд при готовых за 0.4 с картах индекса.
    #
    # Сверяем штампы НАПРЯМУЮ, а не пробным вызовом `перепривязать`: тот при
...
def _прогрев() -> None:
    ...
    # ПОРЯДОК ЭТАПОВ ПЕРЕВЁРНУТ (2026-09-03). «Корпус» — это загрузка склада
    # `state.json` на 363 МБ, и она стояла ПЕРВОЙ: генерация ждала её пять с
    # лишним секунд, хотя больше в ней не нуждается. Отбору от склада нужен был
    # список включённых книг, а он лежит в `active.json` на 1.4 КБ
    # (`nlbridge.активные_книги`); сами тексты пула заменила колонка `src`
    # (`nlindex.маска_книг`).
    #
    # Склад по-прежнему греется — его ждут панель корпуса, заливка книг и
    # чистка, — но ПОСЛЕ карт индекса: первым готовится то, ради чего человек
    # нажимает ↵.
    # Склад — СВОИМ потоком, параллельно (разбор ниже).
    # СКЛАД НА СТАРТЕ БОЛЬШЕ НЕ ГРУЗИТСЯ ВОВСЕ (2026-09-03).
    #
    # Его ждали трое, и ни один больше не ждёт: маска активного пула ушла на
    # колонку `src`, история — на номера строк, панель корпуса — на опись
    # (`nlbridge.опись_книг`, 7 КБ рядом). Остались заливка книг, чистка и
    # статистика — то, что открывают руками.
    #
    # А греть его «на всякий случай» оказалось дорого: разбор `state.json` на
    # 363 МБ — один непрерывный `json.loads`, он держит GIL, и первая
    # генерация ждала пять секунд, считаясь сама за 0.34. Фоновый поток тут не
    # помогает: поток не спасает от GIL, он только прячет виновника.
    #
    # Теперь склад грузится по первому НАСТОЯЩЕМУ обращению — `_nl()`. Платит
    # за это тот, кто открыл панель заливки или статистику, и платит один раз.
    #
    # Опись пишется там же, где склад впервые дочитан (см. `_nl`).
    # ПОЧЕМУ ОТДЕЛЬНЫМ ПОТОКОМ, А НЕ ЭТАПОМ В ОЧЕРЕДИ (2026-09-03). Складом
    # была ПЕРВАЯ ступень прогрева, и генерация ждала её пять с лишним секунд —
    # хотя с переходом на колонку `src` он ей больше не нужен вовсе: включённые
    # книги читаются из `active.json` (1.4 КБ), тексты пула не нужны. Ждут склад
    # панель корпуса, заливка, чистка и статистика — то, что открывают руками и
    # заметно позже первого ↵. Загрузка `state.json` на 363 МБ — один
    # непрерывный `json.loads`, он держит GIL и тормозит соседей; пусть тормозит
    # в стороне, а не в очереди перед лентой.

    этап("карты индекса", карты)
    этап("перепривязка истории", _перепривязать_историю)

    def история():
        """РАЗОВЫЙ ПЕРЕВОД ИСТОРИИ В НОМЕРА СТРОК.

        Показанное вычитается из выдачи по номеру строки в индексе, и номер
        пишется при показе (`corpus.mark_shown`). Записям, снятым до
        2026-09-03, его не хватает — и пока хоть у одной не хватает, история
        переводится словарём «текст → номер» на 2.3 млн ключей: 5.4 секунды при
        КАЖДОМ запуске.

        Проход строит словарь ОДИН раз, дописывает номера и сохраняет. Дальше
        словарь не нужен никогда: у всех записей есть номер, а у сирот его и не
        должно быть — их строк в корпусе нет.

        Ничего не осталось без номера — не делаем ничего, и словарь не
        строится вовсе."""
        idx = nlindex.load()
        if idx is None:
            return
    ...
    # берётся из `active.json` (1.4 КБ), а тексты пула заменила колонка `src`.
    # Ждут его панель корпуса, заливка книг, чистка и статистика — то, что
    # открывают руками и заметно позже первого ↵. Загрузка `state.json` на
    # 363 МБ — один непрерывный вызов `json.loads`, он держит GIL и потому
    # блокирует запросы целиком; на первом месте она стоила первой генерации
    # пяти секунд.
    _ПРОГРЕВ["готов"] = True
...
def _nl():
    """Хранилище корпуса. Грузит его, если ещё не грузили.

    ЛЕНИВО С 2026-09-03: разбор `state.json` на 363 МБ стоит пяти секунд и
    держит GIL, а на старте его больше никто не ждёт (разбор — в `_прогрев`).
    Первый, кто действительно спросит склад, за него и платит; опись книг
    (`nlbridge.записать_опись`) обновляется тут же, чтобы панель дальше
    отвечала без него."""
    был = nlbridge.store_if_ready() is not None
    s = nlbridge.open_store()
    if not был and s is not None:
        nlbridge.записать_опись(s)
    return s
...
def _подписать_источники(shortlist) -> None:
    ...
    if idx is None or getattr(idx, "src", None) is None:
        return
    try:
        # НОМЕР СТРОКИ БЕРЁМ ИЗ САМОЙ СТРОКИ (2026-09-03). Здесь строился
        # словарь «текст → номер» на 2.3 млн ключей — 5.4 секунды, — и строился
        # он при КАЖДОЙ генерации, ради одной лишь подписи книги. Это был
        # последний потребитель словаря на горячем пути: маска пула ушла на
        # колонку `src`, история — на номера строк, а подпись осталась.
        #
        # Движок кладёт номер в строку полем `_ном` (`nlindex._строки`). Словарь
        # остаётся запасным путём — для строк, пришедших без номера (старый
        # бандл в браузере, чужой вызов), — и строится только если такие есть.
        карта = None
        if any(not isinstance(r.get("_ном"), int) for r in shortlist):
            карта = nlindex.text_ids(idx)
        номера = getattr(idx, "sources", []) or []
        имена = {}
        активна = {}
        # ИМЕНА КНИГ — ТОЛЬКО ЕСЛИ СКЛАД УЖЕ ДОЧИТАН (2026-09-03). Здесь стоял
        # `_nl()`, который ЖДЁТ загрузку `state.json` на 363 МБ, и первая
        # генерация платила за это пять секунд — ради подписи, без которой
        # строка прекрасно читается. Номер книги у строки известен и так
        # (колонка `src`), не хватает лишь её имени; оно появится со следующей
        # генерации, когда склад дочитается сам.
        # ИМЕНА — ИЗ ОПИСИ, а склад только если он и так уже в памяти. Опись
        # (7 КБ рядом) держит те же имена; без неё подпись показывала бы
        # внутренний идентификатор книги — то есть врала бы человеку строкой,
        # которую он никогда не видел.
        _склад = nlbridge.store_if_ready()
        книги = (_склад.list_corpora() if _склад else None) or nlbridge.опись_книг() or []
        for c in книги:
        ...
        for r in shortlist:
            ...
            if not isinstance(i, int) or i < 0:
                i = карта.get(r.get("text")) if карта is not None else None
            if i is None:
                continue
            s_i = int(idx.src[i])
            if 0 <= s_i < len(номера):
...
def api_generate():
    ...
    # «ЕСТЬ ЛИ ОТКУДА БРАТЬ» — БЕЗ ЗАГРУЗКИ СКЛАДА (2026-09-03).
    #
    # Здесь стояло `_nl() is not None`, а `_nl()` ЖДЁТ загрузку `state.json` на
    # 363 МБ — 5.3 секунды, которые первая генерация платила за ответ «да».
    # Индексному пути склад не нужен вовсе: включённые книги читаются из
    # `active.json` (1.4 КБ), тексты пула заменила колонка `src`. Значит и
    # спрашивать надо дёшево: есть индекс и есть хоть одна включённая книга.
    #
    ...
    # отдаёт 2.3 млн текстов, словарь «текст → номер» переводит их в номера
    # строк. Замер этого пути: склад 5.3 с + словарь 5.4 с + маска 1.5 с.
    # Двенадцать секунд из двенадцати с половиной, которые уходили на прогрев.
    #
    # Номер книги у каждой строки лежит в индексе колонкой `src` — та же маска
    # из неё берётся за 0.01 с. Сверено побитово на четырёх раскладах
...
def _import_worker(payload: list) -> None:
    ...
    try:
        ...
        if added:
            ...
            _nl().flush()
            _forget_pool_mask()
            _обновить_опись()          # состав книг изменился — слепок тоже
...
def _обновить_опись() -> None:
    """Переписать опись книг после изменения состава. Зовётся рядом с
    `_forget_pool_mask`: то и другое — про «состав книг изменился»."""
    s = nlbridge.store_if_ready()
    if s is not None:
        nlbridge.записать_опись(s)
...
def _скрытое_в_пуле(idx, маска_пула) -> int:
    """Сколько строк активного пула уже показано. По номерам, без словаря."""
    try:
        номера, хвост = CORPUS.скрытые_номера(nlindex.штамп(idx))
        м = nlindex.маска_истории(idx, номера, хвост)
        return int((м & маска_пула).sum())
    except Exception:
        return 0
...
def api_nl_state():
    # ПОКА СКЛАД ГРУЗИТСЯ — ОТВЕЧАЕМ ИЗ ОПИСИ (2026-09-03).
    #
    # Эту панель интерфейс спрашивает при открытии окна, а `_nl()` ЖДЁТ
    # разбора `state.json` на 363 МБ. Его `json.loads` — один непрерывный
    # вызов, он держит GIL, и первая генерация ждала шесть секунд, считаясь
    # сама за 0.34. Опись (`nlbridge.опись_книг`) — маленький файл рядом с тем
    # же составом книг; флаги активности берутся из `active.json`, числа пула
    # считаются по колонкам индекса — то есть по тому же, из чего генерация и
    # набирает.
    #
    ...
    if nlbridge.store_if_ready() is None:
        ...
        if книги and idx is not None:
            ...
            if маска is not None:
                скрыто = _скрытое_в_пуле(idx, маска)
