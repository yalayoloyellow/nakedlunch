...
# СКОЛЬКО ИДЁТ КАЖДЫЙ ЭТАП — ЗАМЕР 2026-08-21, а не догадка: корпус 4.8 с,
# карты индекса 11.9 с, перепривязка истории 0.0 с. Числа нужны, чтобы показать
# ДОЛЮ, а не крутить бесконечный кружок: «идёт» без остатка — это то же
# молчание, только с анимацией. Разъедутся — экран покажет неверный процент, но
# не соврёт про этап, поэтому подпись всегда настоящая, а процент оценочный.
_ВЕС_ЭТАПА = {"корпус": 5.0, "карты индекса": 12.0, "перепривязка истории": 0.5}
...
def _прогрев_статус() -> dict | None:
    ...
    return {"id": "warmup", "label": "Запуск",
            ...
            "done": int(прошло), "total": int(всего), "pct": 100 if _ПРОГРЕВ["готов"] else pct,
...
@app.post("/api/line/trim")
def api_line_trim():
    """«Короче на слово» — рез строки по слову СЛЕВА (2026-09-01).

    ЗАЧЕМ РОУТ, А НЕ ПРАВКА НА ФРОНТЕ. Отрезать слово из текста фронт умеет и
    сам, но решает не текст, а СВИТА: после реза строка обязана по-прежнему
    проходить те ворота, по которым её отобрали. Мат считается словарём корней,
    знаменательные слова — морфологией, внутренняя рифма и перекличка — теми же
    функциями, что при печке. Всё это живёт в ядре, и второй копии правил на
    фронте быть не должно.

    ПОЧЕМУ СЛЕВА. Замер на 40 000 строк: отрезать ПЕРВОЕ слово меняет
    рифмо-ключ у 0.98% строк, ПОСЛЕДНЕЕ — у 73.1%. Ключ берётся с последнего
    знаменательного слова, поэтому голова свободна, а хвост держит рифму,
    клаузулу и ярус разом.

    Ответ честный в обе стороны: `{можно: false, почему: "..."}` — это не
    ошибка, а объяснение погасшей ручки."""
    payload = request.get_json(force=True, silent=True) or {}
    текст = str(payload.get("text") or "")
    if not текст.strip():
        return {"можно": False, "почему": "пустая строка"}
    # Ворота берём из тех же настроек, что и генерацию: ручка обязана уважать
    # ровно то, что стоит на панели сейчас.
    knobs = clean.knobs(clean.knobs_from_profile(
        {"name": "рез", "mode": payload.get("mode"), "params": payload.get("params"),
         "полосы": payload.get("полосы"), "доли": payload.get("доли")}))
    import сдвиг
    return сдвиг.рез(текст, knobs)
