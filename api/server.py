...
def _как_чистит(текст: str) -> str:
    ...
    ЗАЧЕМ НЕ ПРОСТО `подрезать_хвост`. Перепривязка ищет сохранённую строку в
    новом корпусе и, не найдя, пробует нормализованную форму. Пока корпус
    правила ТОЛЬКО подрезка, хватало её одной. Чистка склада правит ещё и
    именем: 1 375 фрагментов на живом складе теряют «Имя Фамилия», и запись
    истории про такой фрагмент стала бы СИРОТОЙ — то есть перестала бы прятать
    уже показанную строку, и та вернулась бы в выдачу.

    Порядок шагов ТОТ ЖЕ, что в `NakedLunchStore.почистить`: сперва имя, потом
    хвост. Обратный дал бы другой результат на строке, где имя стояло в конце.

    Идемпотентно, как и требует `перепривязать`: обе части идемпотентны, значит
    и связка."""
    from nlsrc.cutter import strip_full_names
    return nlindex.Index.подрезать_хвост(strip_full_names(текст))
