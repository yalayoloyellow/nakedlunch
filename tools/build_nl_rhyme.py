...
_ВНУТР_СЛОВО = re.compile(r"[а-яё]{3,}")
# Память «слово → (знаменательное?, лемма, рифмо-ключ)». Словарь корпуса —
# сотни тысяч слов против четырнадцати миллионов вхождений: без памяти бэкфил
# внутренней рифмы стоил бы часы, с ней — минуты.
_ВНУТР_ПАМЯТЬ: dict = {}
...
def _внутренняя_рифма(morph, acc, text: str) -> bool:
    ...
    слова С РАЗНЫМИ ЛЕММАМИ, оба от трёх букв, оба знаменательные
    (_CONTENT_POS — тот же канон, что у рифмо-хвоста), с одинаковым непустым
    рифмо-ключом. Каждое ограничение снято с шума живой пробы:
      · без знаменательных лезли «как/так», «для/меня», «кто/что» — служебные
        слова рифмуются тривиально;
      · без разных лемм лезло «науке/науки» — повтор слова не рифма, а
        тавтология, и её проект казнит отдельно.
    Замер на боевом корпусе после обоих фильтров: 2.9% строк, 67 549
    («перешли/уйти», «головой/рукой», «механической/органической»).

    Ключ — тем же `_word_rhyme_key`, что у клаузул и подсказок: своего понятия
    рифмы у этого правила нет.

    НАДГРОБИЕ: СПАНЫ ДЛЯ ПОДСВЕТКИ ЖИЛИ ЗДЕСЬ ОДИН ДЕНЬ (2026-08-27). По «да»
    владельца правило возвращало позиции рифмующихся слов, кэш нёс «inner_sp»,
    индекс — inner_off/inner_pos, лента подчёркивала. Владелец посмотрел
    живьём и снял: «без спанов — с ними наоборот читать сложнее». Вырезано
    ЦЕЛИКОМ, а не спрятано: поле, которое никто не читает, — мёртвый груз
    (у записей кэша, посчитанных в тот день, «inner_sp» ещё лежит — печка его
    не читает, следующая пересборка сотрёт). Флаг и ворота живут."""
    ключи: dict = {}
    for w in set(_ВНУТР_СЛОВО.findall(text.lower())):
        з = _ВНУТР_ПАМЯТЬ.get(w)
        if з is None:
            p = morph.parse(w)
            if not p or p[0].tag.POS not in _CONTENT_POS:
                з = (False, "", "")
            else:
                з = (True, p[0].normal_form, _word_rhyme_key(acc, w, w))
            _ВНУТР_ПАМЯТЬ[w] = з
        годно, лемма, ключ = з
...
# ВСЕ СЛОВА, А НЕ ТОЛЬКО ПОСЛЕДНЕЕ (2026-08-30). Слово владельца: «рифма
# так-то не только в конце существует. Может быть и первым, и вторым, и
# третьим любым словом. В этом и суть».
#
# ЧТО СЧИТАЕТСЯ. Для каждого ЗНАМЕНАТЕЛЬНОГО слова строки — его рифмо-ключ и
# позиция от конца В СЛОВАХ (0 — последнее слово строки, 1 — предпоследнее).
# Ключ повторился в строке — держим БЛИЖАЙШЕЕ к концу вхождение: чем ближе к
# паузе, тем слышнее рифма, а два вхождения одного ключа рифмуют одинаково.
#
# ПОЧЕМУ ПОЗИЦИЯ ХРАНИТСЯ, А НЕ СЧИТАЕТСЯ ПОТОМ. На ней стоит целая ось ручки
# («конечная · внутренняя · начальная»), и замер объясняет, зачем она: если
# рифмующее слово может стоять где угодно, 7.6% пар физически не слышны — оба
# слова тонут в середине длинных строк («попала» пятое с конца рифмует
# «бежала» шестое). Ограничение последними тремя словами убирает мусор в ноль,
# сохраняя 57% выгоды. Значит позиция — не украшение, а признак отбора.
#
# ЦЕНА ПОЧТИ НУЛЕВАЯ: ключи всех слов УЖЕ считались ради флага «внутренняя
# рифма» (`_внутренняя_рифма` выше) и выбрасывались. Замер: +37 с на весь
# корпус, потому что дорогое — акцентуатор и разбор — оплачено памятью
# `_ВНУТР_ПАМЯТЬ`.
_СЛОВО_С_МЕСТОМ = re.compile(r"[а-яёa-z0-9]+", re.I)


def _ключи_слов(morph, acc, text: str) -> dict:
    """{рифмо-ключ: позиция от конца в словах} по знаменательным словам."""
    слова = _СЛОВО_С_МЕСТОМ.findall(text.lower())
    итог: dict = {}
    последний = len(слова) - 1
    for i, w in enumerate(слова):
        if len(w) < 3 or not _ВНУТР_СЛОВО.fullmatch(w):
            continue
        з = _ВНУТР_ПАМЯТЬ.get(w)
        if з is None:
            p = morph.parse(w)
        ...
        if not годно or not ключ:
            continue
        поз = последний - i
        # ближайшее к концу вхождение ключа — оно слышнее
        if ключ not in итог or поз < итог[ключ]:
            итог[ключ] = поз
    return итог
...
def _поля_фрагмента(morph, acc, text: str) -> tuple[dict, bool]:
    ...
    # `wk` — рифмо-ключи ВСЕХ знаменательных слов с позицией от конца
    # (2026-08-30). Флаг `inner` из него не выводится: он говорит «в строке
    # есть внутренняя рифма», а это ДВА слова с одним ключом, и в `wk` такой
    # ключ лежит один раз (хранится ближайшее к концу вхождение). Два поля —
    # два разных вопроса, и выводить одно из другого значило бы соврать.
    return {"key": key, "span": span,
            "inner": _внутренняя_рифма(morph, acc, text),
            "wk": _ключи_слов(morph, acc, text),
...
def внутр_потоком() -> int:
    """Дописать поле «внутренняя рифма» записям, у которых его нет (2026-08-27).

    Одноразовый бэкфил при вводе колонки: 2.3 млн записей кэша посчитаны до
    правила. Дальше поле пишет `_поля_фрагмента` на каждой новой записи, и
    этот режим больше не нужен — но остаётся, как --rekey: правило внутренней
    рифмы может поменяться, и пересчитать надо будет только его, не трогая
    ударения. Записи, у которых поле уже есть, не пересчитываются — повторный
    запуск бесплатен."""
    import кэш
    if not кэш.есть_строчный():
        raise SystemExit("нет построчного кэша — сперва: build_nl_rhyme.py --jsonl")
    _write_status(0, "running", "внутр")
    morph = pymorphy3.MorphAnalyzer()
    acc = акцентуатор()
    t0 = time.time()
    новых = с_рифмой = 0
    with кэш.Писатель() as п:
        for текст, поля in кэш.поток():
            ...
            п.запиши(текст, поля)
            if п.записано % 500_000 == 0:
                print(f"  {п.записано}  ({time.time()-t0:.0f}с) — дописано {новых}",
                      flush=True)
                _write_status(п.записано, "running", "внутр")
    print(f"внутренняя рифма: дописано {новых}, всего с рифмой {с_рифмой} "
          f"({с_рифмой/max(п.записано,1)*100:.1f}%) за {time.time()-t0:.0f}с",
          flush=True)
    _write_status(п.записано, "done", "внутр")
    return п.записано


def слова_потоком() -> int:
    """Дописать записям кэша ключи всех слов (`wk`) — бэкфил 2026-08-30.

    Тот же приём, что у `внутр_потоком`: 2.3 млн записей посчитаны до правила,
    и пересчитывать ударения ради нового поля незачем — дорогое уже оплачено
    памятью `_ВНУТР_ПАМЯТЬ`. Записи с полем не трогаются, повторный запуск
    бесплатен.

    ЗАЧЕМ ПОЛЕ. Рифма перестаёт жить только в конце строки: пара может
    сойтись любым знаменательным словом, а позиция от конца становится
    отдельной осью отбора («конечная · внутренняя · начальная»)."""
    import кэш
    if not кэш.есть_строчный():
        raise SystemExit("нет построчного кэша — сперва: build_nl_rhyme.py --jsonl")
    _write_status(0, "running", "слова")
    morph = pymorphy3.MorphAnalyzer()
    acc = акцентуатор()
    t0 = time.time()
    новых = ключей = 0
    with кэш.Писатель() as п:
        for текст, поля in кэш.поток():
            if "wk" not in поля:
                поля = {**поля, "wk": _ключи_слов(morph, acc, текст)}
                новых += 1
            ключей += len(поля.get("wk") or ())
            п.запиши(текст, поля)
            if п.записано % 500_000 == 0:
                print(f"  {п.записано}  ({time.time()-t0:.0f}с) — дописано {новых}",
                      flush=True)
                _write_status(п.записано, "running", "слова")
    print(f"ключи слов: дописано {новых}, всего ключей {ключей} "
          f"({ключей/max(п.записано,1):.2f} на строку) за {time.time()-t0:.0f}с",
          flush=True)
    _write_status(п.записано, "done", "слова")
    return п.записано
...
def main() -> int:
    ...
    parser.add_argument("--слова", dest="слова", action="store_true",
                        help="дописать ключи всех слов (wk) записям без него — "
                             "см. слова_потоком()")
    parser.add_argument("--внутр", dest="внутр", action="store_true",
                         help="дописать поле «внутренняя рифма» записям без "
                              "него (бэкфил при вводе колонки или смене "
                              "правила) — см. внутр_потоком()")
