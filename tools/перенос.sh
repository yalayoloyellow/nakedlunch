...
#
# ЗАЧЕМ ОН ЕСТЬ. Это перенос СТАРОЙ установки из исходников вместе с её
# корпусом и индексом. Готовые переносимые релизы в GitHub Releases этого не
# требуют: они запускаются без Python, а книги на новой машине добавляются
# через окно.
#
...
упаковать() {
  ...
  tar -cf "$dst" \
      --exclude='*/записи/*' \
      --exclude='data/telegram.json' \
      --exclude='data/telegram.json.lock' \
  ...
  print -r -- "Telegram-токен, его замок и запись живого окна не включены."
  print -r -- ""
