...
def build() -> int:
    ...
    # СПРАШИВАЕМ У ЗАГРУЗЧИКА, А НЕ У ФАЙЛА (2026-08-18). Сторож стоял на
    # `filters._NL_RHYME_PATH.exists()` — то есть на СТАРОМ `nl_rhyme.json`, а
    # читает эта же строка через `кэш.читать_всё()`, который предпочитает
    # построчный `nl_rhyme.jsonl`. Убери старый файл — и здесь получалось
    # «печь нечего» при полном кэше рядом, то есть та же глухая петля, что в
    # надгробии выше, только с другого конца: не «кэш не загрузился», а
    # «сторож не признал загруженное». Теперь вопрос один и адресован тому,
    # кто читает.
    import кэш
    ...
    # записей даёт 4.2 ГБ на корпусе владельца. Плюс navec (1.5 ГБ у сервера
    ...
    # 0.9 с. Проходов теперь два с половиной (счёт + колонки + сцепка) вместо
    # одной загрузки — плюс двенадцать секунд, минус четыре гигабайта.
    #
    # `filters._NL_RHYME` уважается, если он УЖЕ загружен (запасной путь без
    # индекса): второй раз читать с диска незачем.
    загружен = filters._NL_RHYME
    ...
    if not n:
        ...
        # файла может не быть вовсе. Печатаем то, куда СМОТРЕЛИ, иначе первая же
        # подсказка уводит от причины (вывод сборки и так уходит в /dev/null —
        # см. надгробие выше; тем более он обязан не врать).
        print(f"кэш ударений пуст — печь нечего (сначала tools/build_nl_rhyme.py); "
              f"смотрел: {кэш.СТРОЧНЫЙ} / {кэш.СТАРЫЙ}", file=sys.stderr)
    ...
    bind, _справка = сцепка_колонка((поля for _т, поля in записи()), n)
    ...
    # ── ЗВУКОПИСЬ: ДВЕ КОЛОНКИ (2026-08-30) ────────────────────────────────
    #
    # Здесь по той же причине, что редкость: перекличка меряется РЕДКОСТЬЮ
    # сочетания согласных в корпусе, то есть свойством корпуса, а не запроса.
    # Проходов два, и второй не свернуть в первый: ожидание для сюрприза
    # (доля перекликающихся пар) само выводится только из полного обхода.
    # Цена замерена на боевом корпусе (2 307 826 строк): частоты троек 29 с,
    # обе шкалы 111 и 224 с в двух прогонах — разброс от загрузки машины.
    # Это самая дорогая ступень выпечки, дороже редкости (6-12 с) и сцепки
    # (30-50 с), и она того стоит: считать её на прогреве сервера значило бы
    # платить эти минуты при каждом старте.
    #
    # Правила, пороги и что отвергнуто замером — в `core/звукопись.py`.
    t = time.time()
    статус(stage="звукопись", done=n)
    import звукопись as _звук
    звук_фон = _звук.частоты_кластеров(т for т, _п in записи())
    echo, dens = _звук.шкалы((т for т, _п in записи()), звук_фон)
    # ДОЛЯ ПАР — В ОТЧЁТ: от неё целиком зависит, где стоят ворота, и без неё
    # «0.32%» нечем объяснить.
    print(f"звукопись: {time.time()-t:.1f}с | ворота проходят "
          f"{100*(echo >= _звук.БАЛЛ_МИН*10).mean():.2f}% | доля перекликающихся пар "
          f"{звук_фон.get('доля_пар', 0):.6f} | плотность без шкалы {(dens < 0).sum()}",
          flush=True)

    # ── ЦЕЛОСТНОСТЬ СЛОВ: КОЛОНКА, А НЕ СКАН ПРИ КАЖДОМ СТАРТЕ (2026-09-01)
    #
    # «Обрывок» — это строка, начатая половиной разрезанного слова («нибудь»,
    # «либо», «таки») или оборванная служебным словом в конце. Свойство
    # принадлежит строке и не меняется до следующей печки, а считалось оно
    # проходом по всем текстам ПРИ КАЖДОМ ЗАПУСКЕ: замер 5.54 с из шести
    # секунд холодных ворот — 93% цены первой генерации.
    t = time.time()
    статус(stage="целостность слов", done=n)
    _цел_начало = nlindex.Index._ОБРЫВОК_НАЧАЛО.match
    _цел_оборвана = nlindex.Index._оборвана
    whole = np.ones(n, dtype=np.uint8)
    for i, (т, _п) in enumerate(записи()):
        if _цел_оборвана(т) or _цел_начало(т):
            whole[i] = 0
    print(f"целостность: {time.time()-t:.1f}с | обрывков {int((whole == 0).sum())}", flush=True)

    # НАДГРОБИЕ 2026-09-01: КОЛОНКА `text_hash`. Заводилась, чтобы прогрев не
    # строил словарь «текст → номер» (5.8 с). Замер на настоящем вызове убил
    # идею: `mask_of` зовут со всем активным пулом, и двоичный поиск по хешам
    # это 2 852 секунды против 7.7 у словаря. Разбор — в надгробии
    # `core/nlindex.py` рядом с `text_ids`.

    статус(stage="постобработка", done=n)
    # Своё время, а не чужое (2026-08-30): `t` перезаписывают ступени выше, и
    # эта строка печатала время звукописи второй раз — то есть врала числом.
    t = time.time()
    ...
    for name, arr in (("banal", banal), ("taut", taut), ("bind", bind), ("mat", mat),
                      ...
                      ("echo", echo), ("dens", dens), ("whole", whole),
                      ("inner", inner)):
    ...
    (tmp / "meta.json").write_text(json.dumps({
        ...
        # ТАБЛИЦА ПЕРЕКЛИЧКИ — ЧТОБЫ ВОРОТА МОЖНО БЫЛО ПРОВЕРИТЬ ВНЕ ПЕЧКИ
        # (2026-09-01). Ручка «короче» режет строку в интерфейсе, и после реза
        # надо заново ответить, держится ли перекличка. Для этого нужен фон
        # редкости троек — но целиком он не нужен: редких троек 6 038 из 6 128,
        # а ЧАСТЫХ, то есть тех, что порог не проходят, всего девяносто. Их и
        # храним — 540 байт вместо словаря; проверка «редкая ли тройка»
        # становится «нет ли её в этом списке».
        "звук_частые": sorted(к for к, n in звук_фон["счёт"].items()
                              if n > _звук.ЧАСТОТА_МАКС * звук_фон["слов"]),
        "звук_доля_пар": звук_фон.get("доля_пар", 0.0),
