...
# ИСПЕЧЁННОЕ ПИШЕТСЯ К ПОЛЬЗОВАТЕЛЮ, А НЕ ВНУТРЬ ПРОГРАММЫ (Раунд 60): в
# собранном приложении папка рядом с кодом доступна только на чтение.
OUT = пути.артефакт("nl_index")
...
# молча падала.
СТАТУС = пути.артефакт("nl_index.status.json")
...
def статус(**поля):
    ...
    try:
        СТАТУС.parent.mkdir(parents=True, exist_ok=True)
        было = {}
        if СТАТУС.exists():
...
#
# Требование: одна крутилка работает по обоим фронтам — слова и сочетания..
...
def build() -> int:
    ...
    # просто двенадцать хороших строк. в работе это выглядело как «везде одна и та же книга», а все правки били по симптомам — разброс между равными, полоса
