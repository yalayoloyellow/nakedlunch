...
# ПРОГРЕСС ВИДЕН СНАРУЖИ (Раунд 56). Владелец: «хочу, чтоб всё чётко
...
#
# Владелец: «ручка банальность должна по обоим фронтам работать одна крутилка».
...
def build() -> int:
    ...
    # перепекается. Молча: вывод сборки уходил в /dev/null. Так у владельца и
    ...
    # просто двенадцать хороших строк. Владелец видел это как «везде одна и та
