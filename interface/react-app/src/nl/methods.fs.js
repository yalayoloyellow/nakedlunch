...
export const fsMethods = {
  ...
  // ПОЧЕМУ СТРОКИ НЕ ВИДНО (Раунд 57). Пользователь: «текста всё же нет, даже без
  ...
  // бесплатным (38 мс против 265 у фильтра, замер на стенде), но пользователь
  ...
  // Пользователь сверил бок о бок со своим SVG-фильтром: «идентично выглядит,
  ...
  fsВарп(t) {
    ...
    try {
      ...
      // потом размытие, потом экспозицию и вес. Пользователь: «пусть вообще всё в
  ...
  // Пользователь 2026-08-02: «если я выбрал по одному слову, он по порядку, по
  ...
  // ТЕКСТ ЕСТЬ СРАЗУ (Раунд 56). Пользователь: «сделай для начала так, чтоб текст
  ...
  // СВОИ НАСТРОЙКИ, А НЕ РЕДАКТОРСКИЕ (Раунд 57). Пользователь: «настройки строфы
  ...
  // WebGL-канвасом 1960×1584 под ними. Пользователь смотрел на него сам: «в твоём
  ...
  // фристайл — это генератор, и только он; пользователь: «чтоб текст был
