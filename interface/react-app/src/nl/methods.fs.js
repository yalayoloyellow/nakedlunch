...
export const fsMethods = {
  ...
  // фильтруется по фактическому наличию в системе — установленное у владельца
  ...
  // РАЗМЕР ПАЧКИ (Раунд 56, ПОСЛЕ ЗАМЕРА НА МАШИНЕ ВЛАДЕЛЬЦА).
  //
  ...
  // вызов, а не за строку». ЭТО ОКАЗАЛОСЬ НЕВЕРНО, и журнал с его машины
  ...
  //
  // ВТОРОЙ ЗАМЕР С ЕГО МАШИНЫ ОПРОВЕРГ И ЭТО. Три точки:
  ...
  // и ждать её владелец будет глядя в пустоту. Раунд 57: было 8. Замерено на
  // его сервере (прогретом): shortlist 8 — 0.25 с, shortlist 200 — 1.3–4.2 с.
  ...
  // ПОЧЕМУ СТРОКИ НЕ ВИДНО (Раунд 57). Владелец: «текста всё же нет, даже без
  // баттерчёрна», «он появился на секунду и опять куда-то пропал». А журнал
  ...
  // прозрачность, и прямоугольник — ровно то, что показал осмотр у владельца:
  ...
  // бесплатным (38 мс против 265 у фильтра, замер на стенде), но владелец
  // посмотрел и сказал прямо: «это очень некрасивое искажение, бредовое».
  ...
  // Владелец сверил бок о бок со своим SVG-фильтром: «идентично выглядит,
  // разницы нет визуально».
  //
  ...
  fsВарп(t) {
    ...
    try {
      ...
      // потом размытие, потом экспозицию и вес. Владелец: «пусть вообще всё в
      // реальном времени обновляет, когда кручу текст».
      //
  ...
  paintLine(t) {
    ...
    // ЗАМЕР ПОКАЗА (Раунд 56). Владелец нажал «строку назад» — генерации там
  ...
  async genFsLines(n) {
    ...
    // меняется, когда владелец работает с текстом».
  ...
  fsFill() {
    ...
    this.genFsLines(сколько).then(function (rows) {
      ...
      // Числа снимаем С МАШИНЫ ВЛАДЕЛЬЦА: у меня их взять негде, а гадать про
  ...
  // Владелец 2026-08-02: «если я выбрал по одному слову, он по порядку, по
  // одному слову идёт. Он не урезает никак».
  //
  ...
  // ТЕКСТ ЕСТЬ СРАЗУ (Раунд 56). Владелец: «сделай для начала так, чтоб текст
  // в принципе был, я хз когда он его подгрузит, минута уже прошла».
  //
  ...
  // СВОИ НАСТРОЙКИ, А НЕ РЕДАКТОРСКИЕ (Раунд 57). Владелец: «настройки строфы
  // во фристайле и в редакторе — это одно и то же одновременно? если одно и то
  // же, надо разделить». Так и было: `linkKnobs(0)` и `linkSpec(0)` последней
  ...
  fsНастройки() {
    ...
    if (st.fsParams == null || st.fsKnobMode == null || st.fsSpec === null) {
      ...
      // правим состояние молча: это не выбор владельца, а первое отделение
      this.setState(своё);
  ...
  // WebGL-канвасом 1960×1584 под ними. Владелец смотрел на него сам: «в твоём
  // тесте ничего в принципе не фризит и близко». И он же по очереди выключил в
  ...
  // фристайл — это генератор, и только он; владелец: «чтоб текст был
  // генеративным, а то прогружается мой документ или текст оттуда каким-то
  // хуем». Показывать во фристайле его же черновик — это не заглушка паузы,
  ...
  // было НИЧЕГО — ни строки, ни объяснения; владелец две минуты смотрел в
  ...
  //
  // Замерено на канвасе владельца (1372×1109): **7.15 мс на вызов** при
