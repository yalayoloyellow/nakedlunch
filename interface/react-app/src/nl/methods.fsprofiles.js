...
export const fsProfileMethods = {
  ...
  PROF_SKIP_STATE: {
    ...
    fps: 0,
    // recLock / chipOpen / juncOpen вырезаны из состояния в Раунде 63, но из
    // ЗАПРЕТА не убираются: в уже сохранённых профилях сцены они лежат, и без
    // запрета applyProfile вернул бы их в живое состояние мусором
    ...
    // Все имена ниже вырезаны из живого состояния (документ и листы —
    // 2026-08-18), но из ЗАПРЕТА не убираются: в уже сохранённых профилях сцены
    // они лежат, и без запрета applyProfile вернул бы их в живое состояние
    // мусором. Это уже случалось дважды — см. tests/test_scene_profile_leak.py.
    ...
    // ВКЛАДКА ПАНЕЛИ НАСТРОЕК (2026-08-18). Её здесь не было, и это стоило
    // владельцу молчаливого отказа: в его сохранённых профилях сцены лежит
    // `cfgTab: "документ"` — имя вкладки настроек РЕДАКТОРА, вырезанного вместе
    // с документом. Профиль возвращал это имя в живое состояние,
    // `render.settings.jsx` знает только ['лента','вид','корпус','лог'], ни одна
    // ветка не совпадала — и ПАНЕЛЬ НАСТРОЕК ОТКРЫВАЛАСЬ ПУСТОЙ. Без ошибки в
    // консоли: просто вкладки сверху и ничего под ними.
    //
    // Профилю сцены нечего решать за вкладку настроек — она не часть сцены.
    // Из запрета не убирать и впредь: старые профили на диске останутся с
    // «документом» навсегда.
    //
    // Это ВТОРАЯ половина починки и не главная. Первая — в
    // `render.settings.jsx`: выбор вкладки падает на первую, если имени нет в
    // CFG_TABS. Запрет закрывает известный путь, зажим — все остальные, включая
    // те, которых мы ещё не видели.
    cfgTab: 1,
    // Раскрыт ли скрытый раздел «обслуживание» в настройках корпуса
    // (2026-08-21). Это положение ЭКРАНА на сейчас, а не настройка: утечь в
    // файл профиля оно не должно по той же причине, что и cfgTab — иначе
    // применение профиля месячной давности само раскрывало бы раздел с
    // необратимой кнопкой.
    обслуживание: 1,
    // ЛЕНТА (2026-08-18). Её здесь не было — недосмотр волны Л: профиль сцены
    // снимает «всё, кроме перечисленного», то есть утаскивал в файл ВСЮ выдачу
    // вместе с курсором и выделением, а применение профиля возвращало на экран
    // чужую ленту месячной давности. Ровно тот баг, который этот список и
    // сторожит.
    lenta: 1, lentaCur: 1, lentaSel: 1,
    // ---- МОДЕЛЬ ГЕНЕРАЦИИ ----
  ...
  //
  // Отчёт: один из профилей настроек пропал..
