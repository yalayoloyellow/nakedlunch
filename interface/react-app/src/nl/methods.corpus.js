...
export const corpusMethods = {
  ...
  // ЧИСТКА СКЛАДА (2026-08-21). Сначала ВСЕГДА сухой прогон: чистка правит и
  // выбрасывает фрагменты, и показать цену до того, как её заплатят, — не
  // вежливость, а единственный способ не потерять корпус по нажатию мимо.
  async cleanCorpus() {
    try {
      this.flash('считаю, что уйдёт… это пара минут');
      var сух = await api.corpusClean(true);
      ...
      var текст = 'Чистка уберёт ' + убыль.toLocaleString('ru') + ' фрагментов из '
        ...
        + ', имён ' + сух.имя_вырезано.toLocaleString('ru') + '.\n\n'
        + 'Это НЕОБРАТИМО. После чистки нужно «прогнать всё заново».';
      if (!window.confirm(текст)) { this.flash('отменено'); return; }
      this.flash('чищу склад…');
      var r = await api.corpusClean(false);
      this.flash('готово: ' + r.было.toLocaleString('ru') + ' → ' + r.стало.toLocaleString('ru')
        + (r.надо_перепечь ? ' · теперь жми «прогнать всё заново»' : ''));
      this.statusSoon();
    } catch (e) { this.flash(e.message); }
  },
