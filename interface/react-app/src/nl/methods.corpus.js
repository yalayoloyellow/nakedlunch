...
// Состав активного корпуса — часть любого ключа префетча. Иначе выключенная
// книга исчезала из новых запросов, но уже набранные из неё строки ещё долго
// жили в буфере ленты и фристайла. Сортировка делает ключ независимым от
// порядка строк в ответе панели; число фрагментов сбрасывает его после
// чистки, изменившей размер книги под тем же id. Ревизия индекса закрывает
// случай, когда строки или расчётные колонки изменились при том же размере.
export function nlSourceKey(nl) {
  var источники = nl && Array.isArray(nl.sources) ? nl.sources : [];
  var активные = источники
    .filter(function (с) { return с && с.active; })
    .map(function (с) { return [String(с.id || ''), Number(с.fragment_count || 0)]; })
    .sort(function (a, b) { return a[0].localeCompare(b[0]); });
  return [String(nl && nl.revision || ''), активные];
}
...
export const corpusMethods = {
  ...
  async statusTick() {
    ...
    try {
      ...
      var прежниеРаботы = this.state.jobs || [];
      var шли = прежниеРаботы.some(function (j) { return j.state === 'running'; });
      var идут = работы.some(function (j) { return j.state === 'running'; });
      var корпусОбновился = прежниеРаботы.some(function (было) {
        if (было.state !== 'running' || (было.id !== 'nl_index' && было.id !== 'clean')) return false;
        return !работы.some(function (стало) {
          return стало.id === было.id && стало.state === 'running';
        });
      });
      ...
      // Чистка меняет состав книг, перепечка — значения индексных колонок.
      // В обоих случаях новый /nl/state меняет ключ префетча и старые строки
      // не успевают попасть на экран после окончания работы.
      if (корпусОбновился) this.reloadNl();
      ...
      // смертью ядра. Человеку показывали «ядро не отвечает · закрой и открой
      // заново»; он закрывал и открывал, и ничего не менялось — причина лежала
      // в коде на диске, а не в мёртвом процессе. Хуже того, кнопка «скопировать
      // отчёт» копирует местный журнал окна, в который эту ошибку никто не
      // записывал: отчёт уходил пустым.
      //
      // Теперь: сервер ответил (`e.живой`) — говорим настоящую причину и не
      // трогаем панель смерти; сервер молчит — прежнее поведение целиком.
      if (e && e.живой) {
        this._statusFail = 0;
        if (this.state.ядроМолчит) this.setState({ ядроМолчит: false });
        var текст = 'ядро ответило ошибкой: ' + (e.message || e);
        if (window.__журналОкна) window.__журналОкна.push(текст);
        if (this._последняяОшибкаЯдра !== текст) {
          this._последняяОшибкаЯдра = текст;
          if (this.flash) this.flash(текст);
        }
        this.statusSchedule(ОПРОС_ПОКОЙ);
        return;
      }
  ...
  //
  // Строка сразу становится занятой и второй клик не принимает. Сам флаг
  // меняется только по ответу сервера: оптимистичная точка на короткое время
  // меняла ключ префетча раньше активного состава на сервере, и ответ старого
  // состава мог оказаться в новом буфере. Быстрый active.json делает такую
  // подмену ненужной; занятость остаётся мгновенным подтверждением клика.
  ...
  // разработчику, а не пользователю. Разбор — надгробие в api/server.py.
  ...
  // кончилась так: «просто всплывает надпись „посчитаю“… и я просто
  ...
  // Курсора по строкам не стало вместе со всем выделением: сказано,
