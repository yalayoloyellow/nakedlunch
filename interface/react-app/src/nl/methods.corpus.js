...
//
// ЗАЧЕМ ЭТОТ ФАЙЛ ПОЯВИЛСЯ (Раунд 39). Владелец 2026-08-02: «есть функции,
// которые просто потеряны и не отображаются в дизайне — в этом проблема
// нашего переноса. Нам надо до конца всё перенести». Инвентаризация нашла
...
// её никто. Владелец: «там был progress bar, привязанный к количеству
// источников — сколько обработано из наличествующих… когда заливаешь книгу,
// этот заполняется… максимально информативно, но не громоздко».
...
export const corpusMethods = {
  ...
  // Владелец: «если он постоянно заполнен — это не очень красиво». Поэтому
  ...
  async statusTick() {
    ...
    try {
      ...
      // не видела, пока владелец не трогал что-нибудь руками: за ночь
  ...
  // ЗАЛИВКА ИДЁТ ФОНОМ (Раунд 56). Владелец: «почему бы книгам сразу не
  // добавиться в список и в фоне не обрабатываться… жмёшь и ждёшь в надежде
  // что сработает, никакой информации что даже нажатие сработало нет».
  //
  ...
  // свою книгу владелец должен с момента выбора файла, а не через минуту.
  ...
  //
  // Владелец 2026-08-05: «жму отключить книгу и непонятно вообще, отключится
  // или нет… иногда после нажатия отключить книгу она не отключается».
  //
  ...
  // core/nlsrc/store.py), интерфейс за это время не менялся ничем, и владелец
  // жал ещё раз. Второй клик переключал книгу ОБРАТНО — выглядело как «не
  // сработало».
  //
  ...
  // Нажатие обязано что-то СКАЗАТЬ (Раунд 56). Владелец: «я прогнать заново
  // запустил, а оно нихуя не отображается, что прогоняет заново». Запрос
  ...
  // минуты словаря частот. Сообщение так и говорит — чтобы владелец не ждал час
  ...
  // Подпись времени: сегодняшнее — часами, всё остальное — датой. Владелец:
  // «показывает одно и то же время, которое не является реальным — либо убери,
  // либо сделай, чтобы всё корректно отображалось». Одинаковым оно было по
  ...
  // Раунд 40. Владелец: «функционал избранного и истории был продуман, он был
  // хороший — ты просто его выкинул. Там должно удалять, добавлять, изменять».
  ...
  // ================= статистика =================
  // Панель статистики живёт своей кнопкой (владелец: «она должна находиться в
  // этой кнопочке статистика, и всё там должно быть красиво подведено»), а
  // цифры тянем при открытии — не на каждый рендер и не на старте.
  async loadStats() {
    // Воронка идёт РЯДОМ со статистикой и с ТЕКУЩИМИ ручками владельца: смысл
  ...
  // О чём тексты. Владелец: «частые темы — это когда я тему забиваю, а я их
  // почти не забиваю. Скорее темы стоит вычислять исходя из того, какие у меня
  // тексты сохранены и что в них находится — по этой штуке мы более
  // объективную картину получим».
  //
  // Считаем по тому, что владелец ОСТАВИЛ: избранное плюс живые листы. Слово
