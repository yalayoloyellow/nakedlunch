...
//
// Отличия от дизайна — только уже принятые решения и расхождения с макетом:
//   - ЗАПИСЬ РАБОТАЕТ. До 2026-08-28 здесь стояло «кнопка неактивная, подпись
//     „запись — фаза 4“, MediaRecorder не трогается, на диск ничего» — враньё,
//     пережившее саму фазу. Это правка комментария, а не новая фича. Как есть:
//     клик → recToggle (methods.fsrec.js), два звуковых отвода →
//     freestyle/rec-audio.js, куски уходят мостом в питон (launch.py, rec_start/
//     rec_chunk) и ложатся файлами в ~/Documents/nakedlunch/записи. Видео и
//     склейка вырезаны: захват окна заменяется скринкастом. Заперта
//     кнопка ровно там, где моста нет, — в обычной вкладке браузера («запись
//     только в приложении»);
//   - номера фаз («фаза 4», «фаза 0») сняты отсюда 2026-08-28 вместе с враньём
//     выше: пятифазный план переноса дизайна закрыт (PLAN.md, «Что закрыто»),
//     и нумерация больше ничего не адресует;
...
//     форма из /api/stanza/profiles (моки дизайна не переносятся);
...
export function renderFsLinePanel(c) {
  ...
  // «Диссонанс» реально управляют строками фристайла — он шлёт те же
  // {mode, params}, — но в панели их не было, и крутить их можно было только
  // из редактора. Список и группы зеркалят канон core/clean.py, поэтому панель
  // худеет вместе с ним: «Связность» ушла из канона 2026-08-21 и пропала
  ...
  return (
    <div data-pa="down" data-po={st.closing === 'fsline' ? '1' : null} onInput={(e) => call(c, 'onCardInput', e)} style={s(fsPanel(st.fsLineOpen))}>
      {/* НАДГРОБИЕ 2026-08-30: ПОЛЕ «ТЕМЫ СЦЕНЫ». Заведено Раундом 57, чтобы
          ...
          ), сервер поле `theme` в запросе игнорирует — и поле на
              ...
              <div key={p.name} data-row="1"
                ...
                <span style={s(CTL)}>
                  {/* Мат ходит от −1 («как есть») — ползунок 0..1 отрезал бы
                      ...
                      («выключено»), пока её не удалили 2026-08-21. */}
...
// Кнопка записи (methods.fsrec.js). Читает ровно одно поле состояния —
// st.recOn, — а доступность спрашивает у моста: файлы пишет питон, и в обычной
// вкладке браузера записывать физически нечем. Заперта — значит заперта:
// подпись говорит почему, клик не делает вид, что что-то началось.
