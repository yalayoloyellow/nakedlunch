...
// Следовательно находится в той же менюшке органично».
//
...
// внутри окна (решение владельца: второй демон потребовал бы своего замка на
// данные, а выигрыш был бы только «можно закрыть окно»). Ход виден в шапке —
...
export const seriesMethods = {
   ...
   *  работ, пока серия идёт: без этого владелец видел бы вчерашние цифры
  ...
  async reloadSeriesState() {
    ...
    try {
      ...
      // цепочке владельца константа врала втрое — «22 минуты» там, где час.
   ...
   *
   *  План считается по ТЕКУЩЕЙ правке, а не по сохранённому: цену владелец
   ...
   *  бэк погнал бы вчерашний вариант, а владелец смотрел бы на сегодняшний. */
  async runSeries() {
    var st = this.state;
   ...
   *  ради которой серия и гонялась: раньше прогон кончался, и владелец шёл
