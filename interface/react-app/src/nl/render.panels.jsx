...
// Везде, где ⌘, работает и Ctrl (2026-08-02, просьба владельца): обработчик
      ...
      <div style={s('flex: 1 1 0; min-width: ' + hdrMin + '; display: flex; align-items: center; justify-content: flex-end; gap: ' + hdrGap2 + 'px;')}>
        ...
        <div style={s(edOnlyFlex + 'align-items: center; gap: ' + hdrGap3 + 'px;')}>
          {/* ---- генерация: строфа и пайплайн одним попапом (Раунд 39) ----
              ...
              значок профиля, — и все три вели в одну тему. Владелец: «кнопки
              расположить удобно», «чтоб кнопки не плодить». Теперь одна:
          ...
          {/* РАУНД 55: три кнопки, три меню. Раунд 39 свёл три контрола в один
              ...
              сборка, конвейер, — и общий попап делал их громоздкими. Владелец:
              «серию отдельной кнопкой и отдельной менюшкой во избежание
              громоздкости… хотя и пайплайн тогда тоже отдельной кнопкой».
        ...
        <div style={s('display: flex; align-items: center; gap: ' + hdrGap3 + 'px;')}>
          {/* ---- индикатор фоновой работы (Раунд 39) ----
              ...
              Владелец: «там был progress bar, привязанный к количеству
              источников — сколько обработано из наличествующих… когда
              заливаешь книгу, этот заполняется». Отдаём кружок работам:
              статистика переехала в ⚙ отдельным разделом. Роут /api/status
              существовал с июля, и не звал его никто.
              Покой — тонкий контур (владелец: «если он постоянно заполнен,
              это не очень красиво»), работа — дуга по проценту, несколько
