...
// владельца (data/settings.json, nl_params на 2026-08-03).
//
// Чего здесь больше нет:
//   «Отбор» — стал бинарным переключателем режима (алгоритм/классика), а не
//     ползунком: владелец 2026-08-03 «я бы сделал переключатель, бинарный».
...
// ФРОНТА и полкой не были — а серия ищет цепочку по имени на полке. Владелец:
...
export const panelMethods = {
  ...
  // Владелец: «когда открывается маленькое окошко, шапка и прочее не
  ...
  //
  // Правило владельца: «не согласен с логикой, что нужно убирать полностью.
...
// ---------------------------------------------------------------------------
// Профиль генерации: строфа + крутилки в одном месте (2026-08-02, владелец:
...
export const genProfileMethods = {
   ...
   *
   *  Полка цепочек при этом остаётся полкой: там СЛЕПКИ, которые владелец
  ...
  pickStanzaForm(name) {
    ...
    // ползунки — то смешение каркаса с настройками, которое владелец разделил
    // на две полки.
    //
