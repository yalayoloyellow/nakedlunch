...
export const PARAM_DEFAULTS = {
  'Мат': -1.0, 'Клаузула': 7, 'Внутренняя рифма': 0, 'Позиция рифмы': 1, 'Перекличка': 0,
  'Ярусы рифмы': 3,
  // 0 «не повторять» (прежнее поведение), 1 «можно» — Раунд 52, хук
  // две ручки сняты 2026-08-29 вместе с темой и генератором
  'Повтор': 0,
};
    ...
    this._genProfT = setTimeout(function () {
      ...
      api.settingsSet({
        ...
        nl_params: { params: st.params || {}, mode: st.knobMode || 'алгоритм',
                     полосы: st.полосы || { слова: '', пара: '', плотность: '' },
                     // доли едут рядом с полосами: не запиши их здесь — полоска
                     // забывалась бы при каждом перезапуске окна, и это ровно
                     // та беда, которую полосы уже проходили
                     // оси долей — те же три, что в каноне ядра
                     доли: st.доли || { клаузула: '', рифма: '', позиция: '' } },
