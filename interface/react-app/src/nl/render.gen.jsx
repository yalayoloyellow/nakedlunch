...
//
// Пользователь 2026-08-04: «я хочу отдельно настроить строфу которую мне надо
...
// которые правят одно звено. Пользователь: «мне нравится что оно в одном попапе,
...
// МЕНЮ 1 — СТРОФА  (⌘↵)
// ============================================================
// Мастерская. Всё, что здесь стоит, относится к ОДНОЙ строфе, которую пользователь
...
function renderKnobShelf(c) {
  ...
  return (
    <div>
      ...
      {/* Бинарный переключатель. Пользователь: «либо одно, либо другое. Если
...
function renderChain(c) {
  ...
  return (
    <div data-pop="1" style={s('display: flex; flex-direction: column; gap: 2px;')}>
      {/* ШАПКА СТРОКИ (Раунд 55, по вопросу пользователя «что за столбец куплет
...
// Свёрнут в строку, потому что берутся за него редко (пользователь: «всё, к чему
...
export function renderPipeMenu(c) {
  ...
  // живой отчёт пользователя: «не подтягиваются существующие пайплайны в серию»).
...
//
// Пользователь: «кривая похожа на RGB-кривую в редакторе, в этом смысл». Отсюда
  ...
  return попап(c, 'series', 396, (
    <Fragment>
      ...
      {/* ЧЕСТНАЯ ПУСТОТА. Серия берёт цепочки С ПОЛКИ по имени — пресет живёт
          ...
          прочерком, и пользователь упирался в стену: «не подтягиваются
