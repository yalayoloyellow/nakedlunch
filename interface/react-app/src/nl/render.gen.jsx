...
//
// Владелец 2026-08-04: «я хочу отдельно настроить строфу которую мне надо
// выдавать по команде… отдельно пайплайн когда я хочу вручную его выдавать…
// и серию отдельной кнопкой во избежание громоздкости и так как она
// автоматизированная… это даст возможность не плодить огромные логики».
//
...
// которые правят одно звено. Владелец: «мне нравится что оно в одном попапе,
// просто не нравится как расположено — в этом и основа непонимания».
...
// МЕНЮ 1 — СТРОФА  (⌘↵)
// ============================================================
// Мастерская. Всё, что здесь стоит, относится к ОДНОЙ строфе, которую владелец
// сейчас получит, и к двум полкам, с которых она берётся. Никаких звеньев и
// никакого вопроса «а к чему это относится» — ровно та путаница, которую
...
function renderKnobShelf(c) {
  ...
  return (
    <div>
      ...
      {/* Бинарный переключатель. Владелец: «либо одно, либо другое. Если
          выбрана классика, то всех вот этих алгоритмических оценок их не
          существует — классический метод нарезок просто вообще без ничего». */}
...
function renderChain(c) {
  ...
  return (
    <div data-pop="1" style={s('display: flex; flex-direction: column; gap: 2px;')}>
      {/* ШАПКА СТРОКИ (Раунд 55, по вопросу владельца «что за столбец куплет
          припев в пайплайне, я не оч понимаю»). Три выпадашки стояли подряд
...
// Свёрнут в строку, потому что берутся за него редко (владелец: «всё, к чему
// обращаешься редко, должно быть стильно скрыто и стильно открываться»).
...
export function renderPipeMenu(c) {
  ...
  // живой отчёт владельца: «не подтягиваются существующие пайплайны в серию»).
...
//
// Владелец: «кривая похожа на RGB-кривую в редакторе, в этом смысл». Отсюда
...
function renderCurve(c) {
  ...
  // точками: иначе на экране ломаная, а в прогоне гладкая кривая — и владелец
  ...
  return попап(c, 'series', 396, (
    <Fragment>
      ...
      {/* ЧЕСТНАЯ ПУСТОТА. Серия берёт цепочки С ПОЛКИ по имени — пресет живёт
          ...
          прочерком, и владелец упирался в стену: «не подтягиваются
          существующие пайплайны, и профиль серии сохранить не могу». */}
      {!цепочки.length ? (
      ...
      {/* ЦЕНА ДО ЗАПУСКА, а не после: «тысячи претендентов на трек» — это
          семнадцать суток, и узнать об этом владелец должен здесь. */}
      <div style={s(РАЗДЕЛ)}></div>
      ...
      {/* ОТКУДА ЧИСЛО. Пока текстов не было — грубая прикидка домена; после
          первого текста — настоящий замер этой цепочки. Разница у владельца
