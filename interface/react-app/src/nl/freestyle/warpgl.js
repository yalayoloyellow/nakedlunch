...
// видеокарте, занятой Butterchurn. Замерено в приложении пользователя, перебором
...
//     сдвиг знаков через transform                   38 мс — оценка: искажение получилось некрасивым. И по существу верно: фильтр
...
// НЕТ: один и тот же текст искажается одинаково. сверка бок о бок с исходным фильтром: выглядит идентично, разницы нет. И отдельно
...
// на пиксель. Пользователь прислал снимок: вместо размытия видны ступеньки и резкие
...
function собрать(gl, тип, код, беда) {
  ...
  if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
    // Молча чёрное недопустимо: консоли в окне пользователя нет, а компилятор
...
// (проверено на машине пользователя — журнал сказал прямо), их делает шейдер.
