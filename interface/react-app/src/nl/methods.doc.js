...
export const docMethods = {
  ...
  // Владелец 2026-08-05: «подрубить адекватную маркдаун разметку со скрытием
  // спецсимволов».
  //
  ...
  // буквы — «а б» там, где машина считает «а а». Смотрит владелец именно на
  ...
  //
  // Владелец, глядя на «а а а а» у власти / мотылька / Каракалла / храма:
  ...
  // владельца стояла на максимуме, а максимум этой шкалы — одна ударная
  ...
  watchCaretRow() {
    ...
    this._caretWatch = function () {
      ...
      // строки, у самого края экрана. Владелец прислал снимок: «вот слева
      // сверху», и раньше говорил про «по две, по три, по четыре».
      //
  ...
  renderRows() {
    ...
    // на листе владельца их сто девятнадцать. Каждая сгенерированная строфа
    ...
    // Заполнитель на месте будущей строфы (2026-08-02, владелец: «то что
    // генерация идет я хочу видеть прям в строке на которую идет эта
    // генерация»). Позиция берётся из insertAt() ПРИ РЕНДЕРЕ, а не
    ...
    // «генерация…» пропала совсем. Владелец: «зачем ты надпись генерация убрал,
    // непонятно же теперь».
    ...
    if (sig !== this._rowSig || !host.querySelector('[data-idx]')) {
      ...
      // генерации пустую съедал блок, и всё вставало ровно; владелец так и
      // описал: «отступы при начале генерации, а после генерации норм».
      //
  ...
  genRowNode() {
    ...
    // нет, а пульсирующую точку владелец отверг. Живость даёт сама строка —
  ...
  rowNode(r, i, s, marked) {
    ...
    gut.textContent = isRole ? '#'.repeat(r.level || 2)
      // Пометка источника («я» / «nl») убрана по просьбе владельца: она
    ...
    // выше первой строки. Владелец прислал два снимка: «вот слева сверху» и
    ...
    // Владелец 2026-08-05: «заголовки и разметки в документе неудобны, мб в
    // маркдаун разметку обычную перейти».
    //
    ...
    // стоит. Регистр и разрядка не трогаются вовсе: это текст владельца, а не
    // элемент интерфейса.
    ...
    var field = this.el('div', isRole
      ...
      // негде. Владелец: «на пустой строке значок ввода не отображается, а на
      // тех, где есть текст, отображается». Высота строки, а не заполнитель:
  ...
  // действии — владелец: «перенос строки на интер перестал работать, генерация
  // скачет по строкам».
  //
  ...
  onDocKey(e) {
    ...
    // генерацию. Отсюда лишняя пустая строка над блоком — владелец: «при
    // принудительной вставке отступ строки сверху делается».
  ...
  // строки. Владелец 2026-08-05: «иногда вижу курсор ввода за листом (его
  // шириной) слева сверху».
  //
  ...
  caretСпасти() {
    ...
    // строки перестроились, диапазон умер вместе со старыми узлами. Владелец:
    // «на комманд интер показывается». Ставим её в ту строку, которую сама
    ...
    if (!sel.rangeCount) {
      ...
      // равно рисует в начале координат контейнера. Владелец: «при комманд
      // интер всё равно эта каретка показывается».
      //
  ...
  onDocClick(e) {
    // ВЫБОР СЛОВ ПЕРЕЖИВАЕТ ПЕРЕМЕЩЕНИЕ КАРЕТКИ (Раунд 57, уточнение владельца:
    // «выбрал слово, жму куда вводить текст, перемещаю каретку — и выделенные
    // слова пропадают, а они должны пропадать только при повторном нажатии на
    // них»). Здесь стоял сброс набора; это была моя догадка, а не его правило.
    ...
    // только что сделанного выделения и схлопывал его — владелец 2026-08-05:
    // «не могу выделять мышкой адекватно текст, сбрасывается выделение сразу».
  ...
  // способ оказался неудобным. Владелец: «убрать это усложнение и добавить
  // выбор тем в настройки строфы через запятую, как кейворды». Тема теперь
  ...
  // в поле t — владелец просил «категоризировать и помечать разные типы».
