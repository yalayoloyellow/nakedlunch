        ...
        return (
          <Fragment>
            ...
            <div style={s('display: flex; align-items: center; gap: 8px; margin-bottom: 12px; flex-wrap: wrap;')}>
              ...
              {/* ПОРЯДОК КНОПОК — ПО ЧАСТОТЕ, А НЕ ПО ИСТОРИИ ПОЯВЛЕНИЯ
                  (2026-08-21). Их осталось три, и каждая отвечает на СВОЙ
                  вопрос владельца, без пересечений:

                    залить книгу   — «у меня новая книга»     (пересчёт сам)
                    почистить      — «в корпусе мусор»        (пересчёт сам)
                    прогнать всё   — «что-то не так, переделай всё»

                  Четвёртой была «пересчитать качество» — снята: она строгое
                  подмножество полного прогона и нужна только в день смены
                  формулы в коде (надгробие в api/server.py).

                  Ни одна больше не требует нажать следом другую: чистка сама
                  цепляет инкрементальный пересчёт (роут /api/nl/clean). */}
              <button onClick={function () { c.cleanCorpus(); }}
                title="Выбросить из корпуса обломки, двойники и мусор указателей, подрезать оборванные хвосты. Сначала покажет числами, что уйдёт, и спросит. Пересчёт запустится сам"
              ...
              <button onClick={function () { c.rebuildRhyme(); }}
                title="Пересчитать с нуля всё: ударения, рифмы, качество, индекс. Нужно, только если что-то разошлось — обычные заливка и чистка пересчитывают себя сами. Идёт час в фоне, прогресс в кружке шапки"
