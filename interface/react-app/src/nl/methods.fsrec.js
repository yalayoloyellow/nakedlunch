...
//
// РЕШЕНИЕ ВЛАДЕЛЬЦА (2026-08-01, дословно): «захват окна плюс режим записи и
// умно продумать режим записи и все заебись и усложнять не надо». Отсюда всё
// устройство модуля:
//
...
export const fsRecMethods = {
  ...
  // ни интерфейс, ни владелец не просят. Поэтому наружу два лица с общим
  ...
  // Сбой не гасит запись: остаток дорожек может быть цел, а решать владельцу.
  ...
  // ---- итог и склейка ----------------------------------------------------
  // Склейка НЕ автомат (решение владельца): исходники не трогаем, кнопка
  ...
  recShowDone(results, ms) {
    ...
    // раньше, но владелец спросил «должна быть папка сохранения» — то есть
