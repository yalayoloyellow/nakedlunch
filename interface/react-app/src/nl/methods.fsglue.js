...
// ЖУРНАЛ ИНТЕРФЕЙСА (Раунд 56). Окно «движок не работает» упёрлись ровно в это: я чинила по догадке, потому что настоящего
...
export const fsGlueMethods = {
  ...
  // ХРОНИКА ФРИСТАЙЛА (Раунд 56). Требование: видеть, что и когда реально загружается, а не чинить вслепую..
  //
  ...
  //
  // Требование: кнопка перезапуска визуализатора — чтобы не перезапускать всё приложение,
  // когда он сбивается..
  //
  ...
  fsRestartEngine() {
    ...
    // собирала ГЛУХОЙ движок и делала ровно то, на что отчёт: ни кнопка, ни перезагрузка не помогали..
  ...
  loadEngine() {
    ...
    import('./freestyle/engine.js').then(function (mod) {
      ...
      // СНИМОК СОСТОЯНИЯ (Раунд 56). Журнал ошибок из окна пользователя пришёл
      ...
      setTimeout(function () {
        ...
        // которого не существовало. отчёт: чёрный экран, картинка мигала несколько кадров и пропадала. — это канвас со значением по умолчанию
  ...
  drawBloom() {
    ...
    // ПОЛЯ ВОКРУГ УМЕНЬШЕННОГО КАДРА (Раунд 56). Отчёт: свечение выходило за невидимую рамку и резко обрывалось.. Так и было: размытие
