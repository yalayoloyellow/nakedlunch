...
// ЖУРНАЛ ИНТЕРФЕЙСА (Раунд 56). Окно пользователя — WKWebView внутри pywebview,
...
export const fsGlueMethods = {
  ...
  // ХРОНИКА ФРИСТАЙЛА (Раунд 56). Пользователь: «ты ж нихуя не видишь, сделай так,
  ...
  //
  // Пользователь: «было бы круто добавить кнопку перезапустить butterchurn… когда
  ...
  fsRestartEngine() {
    ...
    // собирала ГЛУХОЙ движок и делала ровно то, на что пользователь пожаловался:
  ...
  loadEngine() {
    ...
    import('./freestyle/engine.js').then(function (mod) {
      ...
      setTimeout(function () {
        ...
        // которого не существовало. Пользователь видел чёрный экран и «мигает на
  ...
  drawBloom() {
    ...
    // ПОЛЯ ВОКРУГ УМЕНЬШЕННОГО КАДРА (Раунд 56). Пользователь: «свечение как будто
