...
//
// ПОЧЕМУ ОНИ ПЕРЕПИСАНЫ (Раунд 40). Владелец: «по истории и по избранному
// функционал был продуман, он был хороший — ты просто его выкинул весь. Там
// должно удалять, добавлять, изменять. Полноценный функционал избранного и
// истории». Так и было: при переносе интерфейса от избранного остались
...
//
// И два решения владельца о том, ГДЕ чему жить:
//   • «настройка истории — можно прямо в историю закинуть, зачем она тут»:
...
export function renderHistPanel(c) {
  ...
  return (
    <div data-pa="down" data-po={st.closing === 'hist' ? '1' : null} style={s(ПАНЕЛЬ + ' width: 360px;')}>
      ...
      {/* Настройки истории живут в истории (решение владельца): здесь видно,
...
export function renderStatsPanel(c) {
  ...
  return (
    <div data-pa="down" data-po={st.closing === 'jobs' ? '1' : null} style={s(ПАНЕЛЬ + ' width: 340px; max-height: 74vh; overflow-y: auto;')}>
      {/* Фоновые работы и статистика — ОДНА кнопка (владелец: «всю статистику
          можно в иконку фоновых работ перенести, она круглая и будет отображать
          всё, не вижу смысла в двух»). Работы стоят сверху и только когда есть
      ...
      {темы.length ? (
        <div style={s('margin-bottom: 12px;')}>
          {/* Владелец: «частые темы — это когда я тему забиваю, а я их почти не
              забиваю. Скорее темы стоит вычислять исходя из того, какие у меня
              тексты сохранены и что в них находится». Считаем по избранному и
              листам — по тому, что владелец ОСТАВИЛ, а не однажды напечатал. */}
      ...
      {/* Выгрузка здесь, а не в настройках: она выгружает СТАТИСТИКУ и корпус,
          и владелец прав — в настройках ей нечего делать. */}
  ...
  return (
    <div data-pa="down" data-po={st.closing === 'black' ? '1' : null} style={s(ПАНЕЛЬ + ' width: 360px;')}>
      ...
      {/* ПОЛЕ БЕЗ УПРАВЛЕНИЯ ИЗ СОСТОЯНИЯ (Раунд 57). Первая версия была
          управляемой (`value` из state), и владелец сказал: «ввод в попапе не
          работает». Управляемое поле в этой панели зависит от того, доедет ли
