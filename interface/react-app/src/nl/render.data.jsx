    ...
    <div data-pa="down" data-po={st.closing === 'fav' ? '1' : null} style={s(ПАНЕЛЬ + ' width: 340px;')}>
      ...
      <div style={s('display: flex; align-items: center; gap: 8px; margin-bottom: 8px;')}>
        ...
        {/* ЗДЕСЬ БЫЛА КНОПКА ＋ (убрана 2026-08-18). Она звала addFavManual,
            ...
            вместе с выделением — Требование: «там сейчас можно бессмысленно
...
// ============================================================
// История: поиск, вставка, копирование, возврат в пул (поштучно и целиком),
// очистка и СРОК ХРАНЕНИЯ — здесь же, а не в настройках.
// «по теме» из этой строки ушло 2026-08-30: тема вырезана 2026-08-29, возврата
// по ней нет ни в одном обработчике, а заголовок его обещал.
// ============================================================
...
// имена файлов: «Pelevin_Nepobedimoe-solnce.CJ_RYw.591928», и про
      ...
      {(ворон.источники || []).length ? (
        <div style={s('margin-top: 8px;')}>
          ...
          {/* ВСЕ книги двумя колонками, а не топ-6 в столбик (2026-08-28,
              Требование: «я вижу только 5, и даже так она дохуя места занимает»).
