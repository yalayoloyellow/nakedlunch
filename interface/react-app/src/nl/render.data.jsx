...
//
// ПОЧЕМУ ОНИ ПЕРЕПИСАНЫ (Раунд 40). Пользователь: «по истории и по избранному
...
//
// И два решения пользователя о том, ГДЕ чему жить:
...
export function renderStatsPanel(c) {
  ...
  return (
    <div data-pa="down" data-po={st.closing === 'jobs' ? '1' : null} style={s(ПАНЕЛЬ + ' width: 340px; max-height: 74vh; overflow-y: auto;')}>
      {/* Фоновые работы и статистика — ОДНА кнопка (пользователь: «всю статистику
      ...
      {темы.length ? (
        <div style={s('margin-bottom: 12px;')}>
          {/* Пользователь: «частые темы — это когда я тему забиваю, а я их почти не
  ...
  return (
    <div data-pa="down" data-po={st.closing === 'black' ? '1' : null} style={s(ПАНЕЛЬ + ' width: 360px;')}>
      ...
      {/* ПОЛЕ БЕЗ УПРАВЛЕНИЯ ИЗ СОСТОЯНИЯ (Раунд 57). Первая версия была
          управляемой (`value` из state), и пользователь сказал: «ввод в попапе не
