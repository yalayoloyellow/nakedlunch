...
def ensure_build() -> bool:
    ...
    if not npm:
        # NODE НЕ НУЖЕН, ЧТОБЫ ПРОСТО ЗАПУСТИТЬ (2026-09-09). Собранный фронт
        # лежит в репозитории, и человек, который поставил программу, а не
        # среду, обязан её запустить без единого дополнительного пакета.
        #
        # Раньше здесь стоял отказ, и он срабатывал не только «когда фронт
        # правда устарел». Git не хранит время правки: после клонирования у
        # исходников и у собранного фронта оно одинаковое, и `stale` выходил
        # то так, то этак — от порядка файлов. То есть свежая установка на
        # машине без npm отказывалась стартовать через раз, по жребию.
        #
        # Теперь: собранный фронт на месте — работаем на нём и говорим об этом
        # вслух. Его нет вовсе — вот тогда отказ, потому что показывать нечего.
        if DIST_INDEX.exists():
            print("nakedlunch: npm не найден — работаю на собранном фронте из "
                  "репозитория. Правил исходники интерфейса? Поставь Node и "
                  "пересобери: cd interface/react-app && npm install && npm run build",
                  file=sys.stderr)
            return True
...
def починить_имена() -> str:
    """Вернуть кириллическим именам файлов составную форму (NFC). Отчёт строкой.

    ЧТО ЭТО ЗА БЕДА. В `core/` шесть модулей названы по-русски: `дочерний`,
    `журнал`, `кэш`, `пути`, `сдвиг`, `доли` и рядом. Юникод хранит «й» двумя
    способами: одной буквой (NFC) или буквой плюс краткой (NFD). Питон ищет
    модуль по имени из исходника, то есть по NFC. Имя на диске в NFD с ним НЕ
    СОВПАДАЕТ, и получается `ModuleNotFoundError: No module named 'дочерний'`
    при файле, который прекрасно виден в Finder и в `ls`.

    ОТКУДА ОНА БЕРЁТСЯ. Не из `git clone` — он кладёт NFC, проверено. А вот
    `tar` и `zip` при переносе раскладывают имена, и тома HFS+ (старые маки,
    внешние диски) раскладывают их всегда, сами. То есть беда приходит ровно
    там, куда мы и целимся: на чужой и на старой машине.

    ПОЧЕМУ ЧИНИМ, А НЕ ЖАЛУЕМСЯ. Переименование обратно в NFC — операция
    безопасная и повторяемая: на нормальном томе она срабатывает и держится, на
    HFS+ не держится, и тогда мы честно об этом говорим. Молчать нельзя:
    сообщение питона указывает на несуществующую причину.
    """
    import unicodedata
    сделано, не_вышло = [], []
    for папка in (HERE / "core", HERE / "core" / "nlsrc", HERE / "api", HERE / "tools"):
        if not папка.is_dir():
            continue
        for файл in list(папка.iterdir()):
            имя = файл.name
            if imya_ascii(имя) or unicodedata.is_normalized("NFC", имя):
                continue
            цель = папка / unicodedata.normalize("NFC", имя)
            try:
                файл.rename(цель)
            except OSError as e:
                не_вышло.append(f"{файл.name}: {e}")
                continue
            if unicodedata.is_normalized("NFC", цель.name) and цель.exists():
                сделано.append(цель.name)
            else:
                не_вышло.append(файл.name)
    if не_вышло:
        return ("имена файлов на этом томе хранятся в разложенной форме, и\n"
                "Python не находит модули, которые прекрасно видны в Finder:\n  "
                + "\n  ".join(не_вышло[:6]) + "\n\n"
                "Так делает файловая система HFS+ — старые маки и внешние диски.\n"
                "Выход: положить программу на том APFS (обычный внутренний диск\n"
                "любого мака с High Sierra и новее) и запустить оттуда.")
    if сделано:
        print(f"nakedlunch: имена файлов приведены к составной форме: "
              f"{', '.join(сделано)}", file=sys.stderr)
    return ""


def imya_ascii(s: str) -> bool:
...
def питон_не_тот() -> str:
    """Причина, по которой этот интерпретатор не потянет ядро. Пусто — годится.

    ПОЧЕМУ ПРОВЕРКА ЖИВЁТ ЗДЕСЬ. Ядро запускается ДОЧЕРНИМ процессом. На
    старом питоне оно падает на разборе файла: `api/server.py` и ещё
    семнадцать модулей написаны синтаксисом 3.10 (`dict | None`), а до 3.10
    это `TypeError` прямо на импорте. Человек при этом видит «Ядро не ответило
    за отведённое время» — то есть таймаут вместо причины.

    Сам `launch.py` намеренно разбирается и на 3.9: иначе это сообщение
    некому было бы напечатать. Проверять его синтаксис незачем — он проверен
    тем, что мы сюда доехали.

    Осечка ровно этого рода и ждёт на старой машине: macOS, которая не берёт
    ничего новее Monterey, несёт системный python3 3.9, а `run.command` до
    правки молча падал на него, когда рядом не было `.venv`.
    """
    if sys.version_info >= МИНИМУМ:
        return ""
    есть = ".".join(str(x) for x in sys.version_info[:3])
    надо = ".".join(str(x) for x in МИНИМУМ)
    return (f"этот Python слишком старый.\n\n"
            f"  запущено на:  {есть}  ({sys.executable})\n"
            f"  нужно:        {надо} или новее\n"
            f"  разработана:  на 3.12\n\n"
            f"Ядро написано синтаксисом 3.10 и на более старом не запустится\n"
            f"вовсе — упадёт на разборе файла.\n\n"
            f"Поставь Python 3.12 (python.org или brew) и собери окружение рядом\n"
            f"с программой — тогда и ярлык, и run.command возьмут именно его:\n"
            f"  cd {HERE}\n"
            f"  python3.12 -m venv .venv && .venv/bin/pip install -r requirements.txt")


def не_потянет() -> str:
    """Причина, по которой ядро НЕ запустится на этой машине. Пусто — запускаем.

    ПОЧЕМУ ЗДЕСЬ, А НЕ ТОЛЬКО В ЯДРЕ. Сторож памяти стоит в `core/filters.py`,
    но срабатывает он в ДОЧЕРНЕМ процессе, на импорте. Оттуда человек увидел бы
    ровно одно: «Ядро не ответило за отведённое время» — то есть таймаут вместо
    причины, а причина осталась бы в журнале, куда он не пойдёт.

    Поэтому тот же расчёт делается здесь, до запуска, и отказ показывается
    страницей в окне. Считает его ОДИН модуль на двоих — `core/память.py`:
    две копии одной арифметики разошлись бы в первый же раз, когда правят одну.

    Индекс на месте — тяжёлый путь не включается вовсе, и спрашивать нечего.
    """
    try:
        sys.path.insert(0, str(HERE / "core"))
        import память
        # Колоночный индекс отвечает через mmap и словарь целиком не читает.
        # Его наличие определяем по описи: без неё грузится словарь.
        if (HERE / "core" / "data" / "nl_index" / "meta.json").exists():
            return ""
        словарь = HERE / "core" / "data" / "nl_rhyme.jsonl"
        if not словарь.exists():
            словарь = HERE / "core" / "data" / "nl_rhyme.json"
        if not словарь.exists():
            return ""                      # грузить нечего — не наша забота
        память.проверить_словарь(словарь)
    except Exception as e:                                       # noqa: BLE001
        # Сам отказ — это НеХватит; всё остальное здесь не повод не запускаться.
        if type(e).__name__ == "НеХватит":
            return str(e)
        return ""
    return ""
...
def main() -> int:
    беда = питон_не_тот()
    if беда:
        _gui_error(беда, title="nakedlunch: не та версия Python")
        return 1

    беда = починить_имена()
    if беда:
        _gui_error(беда, title="nakedlunch: том раскладывает имена файлов")
        return 1
    ...
    беда = не_потянет()
    if беда:
        _gui_error(беда, title="nakedlunch не потянет эту машину")
        return 1
    ...
    def cleanup():
        # ГАСИМ РОВНО СЕРВЕР, А НЕ ДЕРЕВО (2026-08-28, долг с 2026-08-05).
        #
        # Сервер запускает своих детей — пересчёт ударений и перепечку индекса,
        # — и раньше они переживали закрытие окна: программы уже нет, а ребёнок
        # ещё пишет в core/data. Теперь их снимает САМ СЕРВЕР, по SIGTERM и
        # atexit (api/server.py, `_гасить_детей_при_выходе`), а `terminate()`
        # ниже — как раз тот SIGTERM. Пять секунд ожидания ему на это хватает
        # с запасом: он даёт детям три.
        #
        # Почему не искать детей отсюда: launch.py пришлось бы звать pgrep, а
        # тот не отличит нашу сборку от запущенной владельцем в терминале
        # руками. И почему нельзя гасить ГРУППУ: в ЭТОМ процессе живёт запись
        # фристайла (ExportApi.rec_*) и её склейка ffmpeg — порванный дубль не
        # восстановить, ровно как в оговорке про `recording` выше.
        ...
        forget_instance()
        # ОТМЕТКА ЧИСТОГО ВЫХОДА — ПО ТОЙ ЖЕ ПРИЧИНЕ И В ТОМ ЖЕ МЕСТЕ
        # (2026-09-02). Она стояла только в `atexit` выше, а сюда не попала —
        # хотя ловушка описана абзацем выше и ради неё сюда перенесли
        # `forget_instance`. Из-за этого метка была не бесполезной, а
        # ПЕРЕВЁРНУТОЙ: нормально закрытое окно уходило через `os._exit`, и
        # следующий запуск читал это как падение (в журнале владельца 30
        # файлов из 30 помечены аварийными, чистых нет ни одного), а
        # неудачный старт возвращался через `return` и записывался чистым.
        # Регистрация в `atexit` остаётся: ранние возвраты из `main` до этого
        # места не доходят. Двойного вызова бояться нечего — в
        # `журнал.закрыть_сессию` стоит замок.
        try:
            журнал().закрыть_сессию()
        except Exception:
            pass
